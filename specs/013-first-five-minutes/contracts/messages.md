...
After: the two Unmute-minted names leave the fill-in list and join a labelled
block in the shape the LiveKit platform-env section already uses.

```
# supplied for you: `unmute dev` sets these locally, and your platform or
# operator sets them at deploy time. You do not fill these in.
# UNMUTE_PUBLIC_URL=
# UNMUTE_OUTBOUND_TOKEN=
```

The emitted README's "Set these before running (values are never written into
the project)" list drops both names for the same reason.

`docs/TELEPHONY.md` drops "**Generate this secret yourself** with a
cryptographically secure password generator" for `UNMUTE_OUTBOUND_TOKEN`, since
