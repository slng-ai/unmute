...
- [x] T021 [US2] **No code change. Confirm only.** Add no `Transport = "sip"` arm: the wizard cannot supply a carrier either way, so a wizard-built phone package stays gated on both targets. Measured 2026-08-16: LiveKit's path gives `connections/phone.yaml: connection "phone" declares no transport…` (`internal/ir/build.go:101`). **Correction (fourth pass):** an earlier version of this task claimed a `sip` arm would degrade the error to a bare `unsupported telephony route`. That was false — `build.go:984-991` would give `transport "sip" declares no carrier … livekit supports: connector with twilio; sip with plivo, telnyx, twilio`, which is arguably better. The arm is still not added here because it changes wizard behaviour and belongs in its own change, not because the error would be worse. Recorded as a follow-up
