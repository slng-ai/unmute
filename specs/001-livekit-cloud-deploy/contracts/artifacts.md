...
| `secret_set = "<project>-secrets"` when the package needs any environment at all, and no such key otherwise | So the manifest describes where its environment comes from, and a deploy that skipped the secrets step fails at deploy time rather than on a call. **Corrected during implementation, 2026-08-12:** this was first written as "when the package declares secrets", meaning the `secrets:` block. That is the wrong gate. `.env.example` lists the whole required-env list either way, and most packages declare no `secrets:` block while still needing `OPENAI_API_KEY`; keyed on the block, those packages deploy with no environment at all, look healthy, and fail on the first call. The gate is the required-env list. A package that needs no environment gets no set and deploys without one, which is the edge case the spec named |
