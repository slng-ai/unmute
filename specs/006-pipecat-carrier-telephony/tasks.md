...
- [X] T037 [US2] Regenerate goldens with `-update-pipecat`, read the diff, and confirm the no-carrier sections still have not moved. Run `make test`
...
- [X] T049 [P] Update `docs/TRANSFERS.md`: the Pipecat cold transfer row gains the carrier route with its dated evidence from T044, the anchored-transfer billing fact, and the rejected-`sip_refer` reasoning. The warm cell keeps saying what it means (Daily documents the pattern, this project does not emit it yet) and gains the note that the carrier leg will carry warm unchanged when that feature lands
- [X] T050 [P] Update `docs/TELEPHONY.md`: the Pipecat telephony section gains the carrier leg beside the Daily-provisioned form, with the helper named as an operator-run artifact and the local-versus-deployed distinction specs/004 established left intact
- [X] T051 [P] Update `docs/user/targets/pipecat.md` and `docs/user/learn/07-phone-calls.md`: the two Daily forms, when to choose each, the four Connection keys, why no SIP credentials exist on this route, and the helper's role. Keep the two-way sync test in `internal/target/user_docs_test.go` green
- [X] T052 [P] Update `docs/user/reference/controls.md` and `docs/user/reference/targets-yaml.md` for the new route triple and the pairing rule, and audit every "not supported" sentence these edits touch so each says whether it means the platform cannot do it or this project does not emit it yet, with a date (specs/004 SC-013)
- [X] T053 [P] Correct the stale daily-sip claims the route change invalidates: `docs/SCHEMA.md` N34's channel clause is superseded by N37 rather than edited in place (T004 did this; verify no other N34 sentence now reads false), and any `docs/` sentence claiming `daily-sip` is Daily-provisioned only. Search for `daily-sip` across `docs/` and fix each hit or record why it is still correct
- [X] T054 Update `docs/REPO_MAP.md` if the helper template changes where a reader should look for telephony emission on the Pipecat driver
- [X] T055 [P] Fill in the outstanding live-run record in `specs/005-telephony-setup/tasks.md` (T010, T011, and the T021 live half), which the requester confirmed on 2026-08-12 as working on real calls for inbound, cold transfer, and warm transfer. This maps to no requirement of this feature and edits another feature's artifacts; it is carried here because the evidence rule is repository-wide and specs/005's file still shows those tasks open while the feature is shipped and proven. Drop it if the requester would rather close it separately
- [X] T056 In the new route row in `internal/target/telephony.go`, lift `Provisional` to the proven tag only for the features T032, T038, and T044 actually exercised, and leave the rest provisional with a one-line note saying what is still unproven. A capability may not be presented as more proven than its evidence
- [X] T057 [P] Run `go test ./internal/generate -run TestPublicExamplesEmitLintCleanPython` and confirm the helper is inside the linted set and clean. Any hand-edited Python in this change is checked with `ty` and `ruff` per CLAUDE.md
- [X] T057a Perform the plain-language review that spec SC-009 names and CLAUDE.md requires, on the compiled carrier example's README rather than on the template: can a reader who has never used Daily or Pipecat Cloud say which steps happen at the carrier, which happen here, and what the helper is for? Fix what the review finds, in `README.md.tmpl`, then regenerate goldens and record what it changed. A review that changes nothing is recorded as such, with what was checked
- [X] T058 Run the whole offline half of [quickstart.md](quickstart.md) and confirm every listed signal, including the byte-identity of the no-carrier build against the T001 baseline
- [X] T059 Run `make fmt`, `make lint`, `make build`, `make test` and confirm zero failures with zero Python. Then run `make smoke` and confirm it passes or skips cleanly when `uv` is absent, and that it never entered the pull request gate
- [X] T060 Diff the final emitted output of every pre-existing example against the T001 baseline and confirm byte-identity for all of them, with the new example the only addition. Enumerate every accepted golden change in the pull request text, read rather than regenerated blind
...
All four are open on purpose. Every capability on the new route therefore stays `Provisional`, and its note says what is missing rather than the generic line: `built and offline-proven; no call has been placed through a carrier trunk yet` (T056). `docs/TRANSFERS.md` carries the same claim in the same words. Nothing in this change presents a capability as more proven than its evidence.

### Two changes after the first pass, 2026-08-13

**The helper's outbound endpoint is gone, at the requester's decision, and so is `UNMUTE_OUTBOUND_TOKEN`.** The token guarded something real: the helper is internet-reachable by necessity, so an unauthenticated call-placing endpoint would let anyone who found the tunnel URL dial through the operator's trunk. But the endpoint did not need to exist. The helper's whole reason to exist is that an *incoming* call needs a room that does not exist yet; dialling out has no such problem, and the platform's start endpoint can ask for a dial-out-enabled room itself. So outbound is now one command against the platform, which is also exactly what the no-carrier form already does, making the two Daily forms differ only where they must.

What changed: `/outbound` deleted from the helper, the token deleted from `HelperEnv` and from the route row's endpoints (now `inbound` and `health` only), the README's outbound section rewritten as a platform command reading the trunk address from the environment so only the destination is ever typed, and `contracts/forwarding-helper.md` amended in place with a dated note rather than left disagreeing with the code. `docs/TELEPHONY.md` and `docs/TRANSFERS.md` now state the absence and why, because "this is the one Pipecat telephony route with no outbound token" is the kind of difference a reader will otherwise assume is an oversight.

**A real bug found while checking that change: an incoming call would have joined nothing.** The helper starts the agent with `createDailyRoom: false`, because it already made the room. That means the platform has no room to hand over, so `create_transport(runner_args, ...)` — which builds the transport from what the runner supplies — had nothing to build from. The body is the only place that room is named.

The bot now constructs `DailyTransport` directly from `body["room_url"]` and `body["token"]` on an inbound carrier session, and every other session (outbound included, where the platform does make the room) still goes through the runner's factory. `room_url` and `token` became required fields on an inbound body, so a malformed one fails by field name instead of joining an empty room, and `DailyTransport` is now imported for the carrier form whether or not a transfer is declared, which it previously was not. Pinned by `TestCarrierInboundJoinsTheRoomTheHelperMade`.
...
Before this feature that was harmless, because `carrier` meant nothing on this transport. This feature is what made it mean something, so it became a silent downgrade of exactly the kind Principle II forbids: an author writes `carrier: twilio`, forgets the connection, gets a green build with no carrier support anywhere, and finds out when a call to their own number goes nowhere.

Closed in `internal/ir/build.go` beside T008's guard: on `daily-sip`, a carrier with no telephony channel now fails naming all three fields the carrier leg needs and stating that a Daily-provisioned number needs none of them. Pinned by a subtest in `TestBuildResolvesPipecatDailyCarrierPlan`. The same looseness exists on the carrier-WebSocket routes and is pre-existing there; it is left alone rather than widened into by this change.

### Phase 3 to 6 findings

**T020, one dependency and one answer dialect.** The helper adds **no** dependency, because T003 found none missing, so `pyproject.toml.tmpl` is untouched and a Daily-only project's dependency list does not move. The hold answer and the failure answer are the route's single carrier's markup, in two small functions carrying a `ponytail:` comment that names the ceiling: a second carrier replaces those two functions and nothing else in the file. Nothing stops that being wrong silently, because the route table grants this route to Twilio only and `buildPipecatDailyCarrier` refuses any other carrier by name.

**T040 was implemented as two checks, not one, and that is a departure worth stating.** The task asks for the transfer's values in "the emitted startup check". Putting the carrier credentials in the bot's process-wide `REQUIRED_ENV` would have broken US1 scenario 5: `require_env()` runs on every session, so `unmute dev` in the browser and `--console` would both refuse to start on a carrier package until the operator supplied Twilio credentials that neither mode reads. That is the opposite of "a non-phone session behaves exactly as today".

So there are two lists and every name is in exactly one. `REQUIRED_ENV` (unchanged mechanism) covers the model keys, `DAILY_API_KEY`, and the transfer destination. `CALL_REQUIRED_ENV` covers exactly what a phone call adds, which is the four carrier names, and is checked in `_carrier_session` the moment the body says this is a call, before the caller is connected. The requirement behind T040 still holds: a missing value fails by name before the call is answered, never as a failed transfer on a paid call. The test asserts the union and also asserts no name is in both and no carrier credential is in the process-wide list.

This mirrors what the carrier-WebSocket routes already do, where the carrier environment is checked by `telephony.py` rather than by `bot.py`, for the same reason.

**T046 and T047, how the seam is tested without a second carrier.** The route table grants `(pipecat, daily-sip)` to Twilio only, and the spec forbids adding a second carrier's row here, so a second-carrier fixture cannot reach `Generate`. The seam test therefore renders the README template directly with a fabricated second carrier, which is the level the seam actually lives at. It asserts the platform half is byte-identical across the two, that the carrier half falls back to generic prose pointing at the route's documentation URL while keeping all four obligations, and that the rendered helper names no carrier at all. Read once by hand as well: the fallback keeps the number, the webhook, the termination address, and the allow-list with its three-day lead, and loses only the console paths.

### Phase 7 findings

**T053, the stale-claim sweep.** `grep -rn daily-sip docs/` found one sentence the route change made false that N37's first draft had not covered: N34's narrowing of N30 at `docs/SCHEMA.md:115`, "the Daily route has none and cannot declare one" (a phone channel). N37 now supersedes that clause too, and says the narrowing still stands on the no-carrier form. N34's own text is unedited, as history. The other N34-adjacent statements (SCHEMA lines 393, 397, 739, 740, 803) were each checked and are still true: "Daily SIP only" and "Pipecat native on Daily SIP" cover both forms.

**T057a, the plain-language review.** Run on the compiled carrier example's README, not the template, against SC-009: can a reader who has never used Daily or Pipecat Cloud say which steps happen at the carrier, which happen here, and what the helper is for? It found four things and all four are fixed.

1. **Nothing named the three parties.** The section opened straight into actions, so "Daily" and "Pipecat Cloud" appeared as bare brand names a first-time reader had no place to put. Added a short **Who does what** list: the carrier owns the number, Daily is the audio bridge, Pipecat Cloud runs the agent, and this project touches none of the carrier's settings.
2. **The helper's reason was one compressed sentence** with a comma splice in the middle of the causal chain. Given its own **Why there is a helper** heading and unpacked: a room per call means a SIP address per call means no fixed address to point a carrier at. It also now says what the helper does *not* do, which is what stops a reader assuming it is part of the agent.
3. **"Two commands, both reading every value from the `.env` you filled in" was false.** The tunnel command reads nothing. Split into two labelled commands, and the second says why it exists (a public address your carrier can reach) and that it goes in a second terminal.
4. **The Quickstart offered browser and console with no signpost.** A reader following the file top-down met two commands that are not the phone path and no hint that the phone path exists. Added one sentence pointing at **Telephony setup**.

Goldens regenerated after the review; the only no-carrier lines that moved are still T006's two and T041's comment.

**T060, byte-identity against the T001 baseline.** Eight of the nine pre-existing examples are **byte-identical**, compared file by file rather than by size. `human-transfer-daily` differs in exactly three files and exactly two ways, both deliberate and both required by tasks in this list: T006's `daily_dialout` summary (in `README.md` and `compile-report.json`) and T041's `sip_refer`-rejection comment (in `bot.py`). No file was added or removed, no environment name changed, no structure moved. `human-transfer-daily-twilio` is the only addition.

One correction to how T001 was captured: the first baseline copy included stale `build/` leftovers from earlier local compiles (`human-transfer/build/pipecat/` from a version of that example that no longer has a Pipecat target, plus `.env` and `telephony.log` files), which showed up as spurious deletions. The comparison was redone from a copy with every `build/` removed first, on both sides. Only the clean comparison is reported above.

### Phase 1 findings

### Phase 1 findings

**T001, the baseline.** All nine pre-feature examples compiled from a copy outside the repository on 2026-08-13, 234 emitted files, manifest of path plus byte size held at `<scratch>/baseline-manifest.txt` for T060. `human-transfer-daily/build/pipecat` is the row that matters: `.dockerignore` 5, `.env.example` 189, `Dockerfile` 1163, `README.md` 6784, `bot.py` 13128, `compile-report.json` 3110, `compose.dev.yaml` 768, `pcc-deploy.toml` 55, `pyproject.toml` 933.

**T002, which golden sections must not move.** `internal/testdata/safe_core`'s pipecat target is `transport: daily-sip` with no carrier, so `pipecat_v1.txt` is *entirely* the no-carrier Daily build. Every one of its 913 lines and all 13 sections (`.dockerignore`, `.env.example`, `Dockerfile`, `README.md`, `bot.py`, `compile-report.json`, `compose.dev.yaml`, `pcc-deploy.toml`, `pyproject.toml`) must stay byte-identical. That is a stronger signal than the task assumed and it needs no section list.

Two lines did move, both deliberate and both T006: the `daily_dialout` summary, in the README's "Account prerequisites" section and in the compile report's `route_prerequisites`. Nothing else in the golden changed, before or after the carrier work.

**T003, what the helper needs and what is already there.** Nothing is missing, so **no dependency is added**. The `runner` extra is unconditional in `collectImportsExtras` and `pipecat-ai[runner]` requires `fastapi>=0.115.6,<1`, `uvicorn>=0.32,<1`, and `python-dotenv` (read from the installed `pipecat_ai` metadata, 2026-08-13). `aiohttp>=3.11.12,<4` is a core `pipecat-ai` requirement, not an extra, and `pipecat.runner.daily.configure` takes an `aiohttp.ClientSession`, so the helper mints rooms and calls the start endpoint over a client the project already has. `pyproject.toml.tmpl` is therefore untouched by this feature.

### Phase 2 findings

**T005a, the scaffold: nothing changes, and here is why.** `scaffold.Data` has no connection field at all (`internal/scaffold/scaffold.go`, `Data` struct), and nothing in the package writes `connections/*.yaml`. So the scaffold cannot produce a carrier target on *any* route, this one included, and its `daily-sip` branches carry no carrier knowledge to correct: `SetTarget` picks the transport and the version, and `RequiredEnv` adds `DAILY_API_KEY`. Both are correct for both Daily forms. A scaffolded telephony target on Pipecat still stops at `requires connection for telephony`, exactly as `scaffold_test.go:382` and `:401` pin, before and after this feature. This is a pre-existing gap in the scaffold, identical on the carrier-websocket routes, and widening the scaffold to author Connections is its own feature.

**T005b, the interactive console: same finding, same reason.** `internal/tui/tui.go` mentions no connection anywhere either, so the transport prompt at line 2164 and the carrier prompt at line 2189 can set the two fields but never the third. Choosing `daily-sip` plus a carrier therefore cannot produce a package the build refuses *for a new reason*: it fails at the same connection gate it fails at today, which is what `tui_test.go:1012` pins through the accessible renderer. Examined, unchanged, recorded.

**T011a, the gating-site audit, and the decision it produced.** Enumerated by grep on 2026-08-13: **22 sites**, not 24. Nine in `README.md.tmpl` (lines 10, 16, 18, 23, 24, 29, 30, 32, 35), eleven in `bot.py.tmpl` (30, 36, 38, 58, 77, 138, 139, 247, 491, 583, 704), one in `Dockerfile.tmpl` (26), one in `pyproject.toml.tmpl` (14), plus `compose.telephony.yaml.tmpl:6` (which only renders for those routes anyway) and four in Go: `pipecat_v1.go:439` (the artifact branch), `pipecat_v1_build.go:158` (carrier deps), `:219` (`buildPipecatTelephony`), `:306` (`inlineEligible`). `pipecat_v1_build.go:784` (`humanTransferTool`) reads the *IR* plan rather than the template data, so it is T012's, not this list's.

**None of the 22 was narrowed, because the new route does not populate the field they read.** `data.Telephony` keeps meaning "carrier-websocket adapter", which is what all 22 already assume, and the Daily carrier leg travels in a separate `data.DailyCarrier` group. Narrowing 22 sites by hand would have to be right 22 times and fails *quietly* when it is not, which the task itself names as this feature's one silent ordering trap. A second field cannot arm any of them, so the trap does not exist rather than being defused. The two Go sites that genuinely needed the new route are the ones that now read `DailyCarrier` explicitly: `renderPipecatFiles` (T021) and `inlineEligible`. T018's artifact-set assertion is still the proof, and it passes for the structural reason rather than by inspection.

**Baseline notes (T001, T002)**: see the Phase 1 findings above.
