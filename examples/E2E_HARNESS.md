...
| `outbound-reminder` | pipecat, livekit | Every runtime value kind at once: input variables from the dispatch, a system variable, a conversation variable, and two webhook tools plus one local tool. | Derived, unverified. Start with `--var customer_id=cus_1042 --var name=Ada --var appointment_time="tomorrow at 3pm"`, then answer "no, can we move it to Friday?" | The greeting already says the name and time, so the variables arrived. Then `reschedule_appointment` fires and the conversation variable `reschedule_to` is saved. |
...
| `subagents` | retest needed | verified | Pipecat completed the fresh booking-desk → appointment-manager → booking-desk flow with carried context and no duplicate transfer. The first LiveKit run exposed B3: reciprocal transfer tools were available during automatic entry inference, so the two exact announcements alternated four times without another caller turn. V3 now hides only `agent_transfer` tools during that inference through LiveKit's native `IGNORE_ON_ENTER` flag; the fresh human retest had one cue per transfer, continued the booking, and logged zero runtime/provider exceptions. The caller did report one repeated phone question, so LiveKit does not yet get a full pass. A direct generated-context probe proved the booking desk receives the phone message plus the successful `lookup_customer` call and `cus_1001` result; an exact three-turn LiveKit text simulation then handed back and asked for the haircut time, not the phone. This is not reproduced as missing tool context; repeat the exact live script once more before marking verified. |
