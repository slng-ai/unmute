...
| `pipecat-human-transfer-twilio` | n/a | partial | Pipecat only. Fresh validate and compile passed. Inbound call `CA0b43a27663586f81861c981784b4e753` reached the local agent, which heard the charge request and ran `send_to_billing` exactly once with `{}`. Twilio accepted the replacement markup and completed billing leg `CA8b76efe3dc246440c48108a88cb69ac6` after 5 seconds. The caller then heard an application error at the documented handback boundary: the markup tried to reconnect to `pipecat.zonal-bison-orange-168`, but that organization has no deployed agents. The original call remained open after the announcement and was ended explicitly to stop charges. The CLI restored the number's original TwiML URL. Human confirmation that both phones exchanged audio is still pending; a full pass also needs one deployed `pipecat` agent and a retest of the fresh-session handback. |
