...
6. On yes, complete immediately and silently with the exact action, booking ID
   when needed, service and slot when needed, and `confirmed` true.
7. On no, complete with action `none`, empty IDs, and `confirmed` false.
