    ...
    with _connect() as database:
        ...
        if customer["normalized_name"] != normalized_name:
            return {
                ...
                "summary": "That phone number belongs to a different name.",
            }
...
def create_booking(customer_id, service, slot_id):
...
def modify_booking(customer_id, booking_id, service, slot_id):
    parts = _slot_parts(slot_id)
    if not parts or parts[1] != service:
...
def _demo():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        unverified = create_booking("cus_missing", "haircut", first_slot)
        assert unverified["status"] == "customer_not_found"
        with _connect() as database:
            assert database.execute("SELECT COUNT(*) FROM bookings").fetchone()[0] == 0

        booking = create_booking(created["customer_id"], "haircut", first_slot)
        ...
        cancelled = cancel_booking(created["customer_id"], booking["booking_id"])
