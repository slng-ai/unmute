...
| [`human-transfer`](human-transfer/) | One phone agent, two ways to reach a person | Cold transfer hands the caller off and the agent drops out; warm transfer holds the caller, briefs a supervisor, then bridges the two. Two targets from one `agent.yaml`: Pipecat over Twilio Media Streams (runs on a laptop) and LiveKit over a Twilio SIP trunk. |
