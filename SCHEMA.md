...
| `merge` | no | `results` (only value) | core | On completion the owner receives the steps' typed results and nothing else (N13). On LiveKit the driver must set `summarize_chat_ctx=False`; TaskGroup summarizes by default. `summary` stays out: managed targets cannot host a summarizer. |
