...
| `human_transfer` | N cold / C warm⁰ | C cold: Daily SIP or provisional carrier REST; warm No¹ | N cold / C warm (Twilio)² | N (blind, conference, SIP REFER)³ | P at the bridge, carrier conditional⁴ | N cold / C warm⁵ | N cold and warm | N blind only⁶ | C undistinguished⁷ |
...
controls:
  ...
  to_human:
    ...
    cold:                      # the shape is a block, not a `mode:` field
                               # (SCHEMA N25). Exactly one of cold:/warm:, and
                               # the block carries the transfer's settings (N27).
...
capacity:
  ...
  max_sessions: 60             # hard admission limit; reject above
...
7. Telephony controls resolve against the matrix for the resolved carrier and transport, never the provider alone. The control vocabulary is `cold_transfer`, `warm_transfer`, `dtmf_send`, `dtmf_receive`, `hold`, `hangup`, `voicemail_detection`, `ivr_navigation`; send and receive are independent capabilities everywhere. `human_transfer` destinations must resolve through the target instance's `destinations:` map to a phone number or SIP URI. The transfer shape is a block name, `cold:` or `warm:`, never a `mode:` field, and `warm.briefing` is free text rather than a mode enum (SCHEMA N25, 2026-08-10; the removed `summary | message | wait` values were Vapi's `transferPlan.mode` vocabulary and mapped to no other target). A managed driver maps the free text onto whichever native briefing mode fits: Vapi into `transferPlan.mode`, ElevenLabs conference mode into its operator message. Both blocks also take `ring_timeout` and `on_unavailable`. Warm-transfer conditions per provider come from the section 2.3 conditions list. Whether Bolna's undistinguished transfer satisfies a `cold:` block is decided per target driver and documented in its diagnostics, not assumed.
