...
- **C1: the shape is the block name, not a field value.** A control declares exactly one of `cold:` or `warm:`. Zero blocks and two blocks both fail at **build** with file:line and the control named. (Tools check this at load because a tool is its own file and a column-zero key scan finds its blocks; a control is nested inside `agent.yaml`, so the check lives where the control is resolved.) This is N19 applied to controls, for the same reason: with `mode: cold` plus a sibling `briefing:`, a warm-only field on a cold transfer is *writable* and needs a cross-field rule to reject it. With the block, it is unwritable and the rule disappears. A block with nothing set is written `cold: {}`, the spelling already established for `client: {}` and `provider_hosted: {}` (SCHEMA §5.2).
- **C2: `destination` stays at the top level.** It answers *who*, which both shapes need and both mean identically. Only *how it runs* moves into the block, mirroring tools where `description` and `input` stay at the top level and only the execution block moves down. `destination` keeps its current contract: a symbolic name resolved through the target instance's `destinations:` map to an E.164 number or a SIP URI, validated by `ir.validDestination`.
...
Block fields, both optional in both blocks:
...
`internal/spec/package.go`. Remove `Mode *string` and `Briefing *string`; add the two blocks, mirroring the `Tool` execution blocks:
...
A `TransferShape() string` method mirrors `Tool.ExecutionKind()`, returning `"cold"`, `"warm"`, or `""`. `unexpectedControlField` (`internal/ir/build.go`) drops `mode`/`briefing` and gains `cold`/`warm` under `ControlHumanTransfer`, so the two blocks on an `agent_transfer` still fail by name.
...
`internal/generate/pipecat_v1_build.go`, `templates/pipecat_v1/bot.py.tmpl`, and `templates/pipecat_v1/telephony_twilio.py.tmpl`. **Not emitted yet** (T7/T8). Route: `carrier-websocket` + `twilio` first (C7).
...
- **V1**: a `human_transfer` control declares exactly one of `cold:` / `warm:`. Zero or two fail at build with file:line and the control named; the zero case shows the `cold: {}` form, because a `cold:` written with an empty body decodes as absent and would otherwise fail as "no block" with no clue why (C1).
...
T1|x|`spec.Control` gains `Cold`/`Warm` blocks and `TransferShape()`; `Mode`/`Briefing` removed; `unexpectedControlField` updated; the no-block error names the `cold: {}` form, because a `cold:` written with an empty body decodes as absent|C1,C2,I.spec.Control,V1,V2,V3
...
T12|x|migration: `internal/testdata/safe_core` moves to `cold: {}`; every test constructing `spec.Control{Mode:…}` or an `ir.Briefing*` constant moves to the block and the free-text field|V3
T13|x|`examples/human-transfer/`: one LiveKit target on the Twilio SIP route showing both shapes, placeholder `+34` destinations, and a README covering the trunk's SIP REFER setting. Registered in `TestPublicExamplePackages`, so `TestPublicExamplesValidateAndGenerate` gates both shapes end to end. Checked beyond that gate by hand: `ruff check`, `ty check`, and importing the emitted `agent.py` against real `livekit-agents`|V6,V7,B4
