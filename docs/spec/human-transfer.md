...
- **C2 (reversed 2026-08-11, SCHEMA N27): `destination` lives inside the block.** The original rule was about *sharing*: both shapes need it, so it sat above them. What that produced was `cold: {}` — a block naming a shape while the field deciding where the call goes sat outside it, which reads as a shape with nothing to configure. The rule that replaces it is about *placement*: above the block is the tool (`kind`, `when`), inside it is the transfer. `destination` is required in both blocks and keeps its contract otherwise: a symbolic name resolved through the target instance's `destinations:` map to an E.164 number, a SIP URI, or an env var name (N26), validated by `ir.validDestination`.
...
`internal/spec/package.go`. Remove `Mode *string`, `Briefing *string` and (N27) `Destination *string`; add the two blocks, mirroring the `Tool` execution blocks:
...
- **V16 (2026-08-11, supersedes the audio-path half of C4/C9/V10/V11 for Pipecat)**: Pipecat warm transfer is a **Twilio conference**, not the two-socket bridge. Live testing showed the bridge is unsustainable: it keeps this process in the audio path for the whole call (tunnel latency the user could hear), leaks its briefing pipeline into the next call in the process, and drops the caller when the supervisor hangs up — and the Pipecat/Daily docs confirm a bot-owned bridge cannot avoid this. So hold, bridge, and teardown move to the carrier: the caller waits in a `<Conference startConferenceOnEnter="false">` (Twilio's own hold music, no `waitUrl`), the person is briefed on their stream and redirected into the same conference (`startConferenceOnEnter="true" endConferenceOnExit="true"`), and this process leaves the audio path. Because redirecting the caller into the conference closes their media socket and ends the session, the whole dial-brief-join runs as a **detached task** (`_run_warm`), not in the pipeline. The `_HoldMixer`, `_AudioBridge`, `_CallerLeg`, and `MixerEnableFrame` are deleted. Guarded by `TestPipecatWarmHumanTransferUsesConference`. The connector route follows (its own §T); `return_to_caller` seeds a fresh session for now, transcript-restore is a follow-up.
- **V15 (2026-08-11, SCHEMA N30)**: a package containing a `warm:` transfer declares `channels.phone.outbound: true`. Warm holds the caller and then dials the destination, so the agent originates calls whatever the channel says, and `outbound: false` beside a warm transfer is the package describing a shape it does not emit. Enforced in `ir.Validate`, one error per channel rather than one per control, because the remedy is a single line either way. Cold is untouched: it reroutes the caller's existing call and originates nothing, so it stays legal on an inbound-only agent. Guarded by `TestV12_WarmTransferRequiresOutboundDirection`, which flips the safe core's shipped cold transfer to warm and asserts the error appears and disappears with the flag (§B B8).
...
T11|x|docs: SCHEMA.md N25 + §4.7 rewrite + §7/§9 rows; `docs/user/reference/controls.md` rewrite; `07-phone-calls.md`, `targets/livekit.md`, `targets/pipecat.md`, ORCHESTRATOR_SHARED_CONFIGURATION.md, and TELEPHONY.md moved off the old spelling|I.docs
...
B8|2026-08-11|`bin/unmute dev examples/human-transfer --target livekit --telephony --to +39…` refused with `--to needs an outbound-capable target; "livekit" has no outbound direction`, and it read as a driver regression: outbound "was completely working before". Nothing had regressed. `(livekit, connector, twilio)` grants `TelephonyOutbound` and emits `POST /telephony/outbound`, and `examples/telephony-hello` dials out on both orchestrators. The example's own `agent.yaml` said `outbound: false`, and had said so since the commit that created it, while its `escalate_to_supervisor` control is a warm transfer that dials the supervisor. So the package validated green while declaring it never places calls, and the CLI gate that reads that declaration did exactly its job. The user's real need was `--to` as a way to test a transfer when the inbound number cannot be reached, which the false declaration silently removed. Found by someone trying to test their own example.|V15: `ir.Validate` rejects a warm transfer on a channel without `outbound: true`, so the contradiction cannot be authored. The example declares `outbound: true` with a comment naming both reasons. SCHEMA N30 records the rule; 07-phone-calls.md tells authors the rule and points at `--to` as the test path when inbound is not available to them.
