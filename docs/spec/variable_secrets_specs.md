...
- C4: **Secrets never flow through templates.** `{{...}}` resolves against variables only. SCHEMA 4.4 already says this for Deepgram ("never route secrets through template variables"); this spec makes it a package-wide rule on every target. Secrets reach the call only through the existing `*_env` slots and the generated auth helpers, and through `os.environ` in local Python handlers.
  - The reason is the destination, not the ceremony. Every template site renders into something that leaves the process: the greeting is spoken, instructions enter the model provider's logs, `inject:` values become tool-call arguments that Langfuse records, and `webhook.path` becomes a URL every proxy keeps. Variables are the speakable channel and secrets are the credential channel, and they are separate because their sinks are.
  - This is also why a secret is never passed into a local handler as a keyword argument, which would put it back in the traced tool-call arguments. The handler reads its own environment instead. A redaction wrapper (`pydantic.SecretStr`) was considered and rejected for the same reason: it protects a credential carried around as a value, and this design does not carry one. The value is read at the point of use and consumed in the same expression.
...
- V9: no package file ever contains a secret value: a secret is a bare name in a list, so there is no field a value could be written into (C3), and `*_env` fields keep rejecting non-env-name values (N19).
- V10 (amended 2026-08-11): an env name referenced by the package but not declared in `secrets:` produces one warning listing each name and the site that references it, stderr exit 0, never an error (C7). The referencing sites are tool `url_env`, `token_env`, mcp `url_env`, model `endpoint_env`, the tracing trio when `tracing:` is present, **and the body of every local handler**, matched with one regexp over `os.environ["X"]`, `os.environ.get("X")`, and `os.getenv("X")` for `UPPER_SNAKE` names. Connection env names are exempt (declared in their own file). Declared-but-unreferenced secrets are silent.
  - The handler scan closes the one gap the original V10 left open: a local handler owns its own request, so its credential is named in Python rather than in a `*_env` field, and an undeclared one reached neither `.env.example` nor the startup check. It failed on the first tool call instead, which is the failure `secrets:` exists to prevent. `ir.Tool.HandlerSource` already carries the file content (`json:"-"`), so the scan is contained in `referencedEnvNames` with no type change and no schema change.
  - It is a text match, not a Python parse: a name in a comment or a dead branch counts. Over-reporting is the chosen direction, since a spurious line costs one extra declaration and a miss costs a live call. Lowercase lookups are skipped, so `os.getenv("debug")` is not mistaken for a credential.
  - The handler-read name still does not enter `REQUIRED_ENV` or `.env.example` on its own. Declaring it does, which is what the warning asks for; inferring it would make the two paths silently different.
...
T5b|x|extend the cross-check to local handler bodies (`os.environ`/`os.getenv` reads)|V10
