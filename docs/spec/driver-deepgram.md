...
- V3: turn thresholds are forwarded on the listen binding's `params`; a turn binding on the integrated role carries settings only, never an outside model (C3; SCHEMA N15 — declared turn placement no longer exists, so no placement warn).
...
- V10: no max-tokens param is ever forwarded (C5); L4 smoke proves the bridge starts and the `Settings` JSON is accepted; safe_core → deepgram emits valid, with only the `speaks_first: user` / interruption-tuning warnings SCHEMA.md §7 allows.
...
T4|.|turn thresholds on listen `params`; drop `eot_*` + warn on non-Flux listen model; semantic_endpointing (turn binding, N15) forwarded|C3,V3
