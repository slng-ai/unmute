...
- V8: `human_transfer` — `mode: cold` native; `mode: warm` on the stable `transferPlan` path requires `carrier: twilio` (the experimental assistant-based warm transfer also works with Vapi numbers/SIP, not emitted); `briefing:` all three map to `transferPlan.mode` `warm-transfer-with-summary`/`-with-message`/`-wait-for-operator-…`. (review-corrected 2026-07-15: warm is not universally Twilio-only.)
