...
| `simple-prompt` | livekit, pipecat | One agent owning five tools, plus Langfuse tracing. The only package that configures a tracing provider. | Derived, unverified. "I need to cancel my appointment." Give a name and phone when asked. | `lookup_customer` then `cancel_appointment`, and a trace visible in Langfuse. Needs the three `LANGFUSE_*` keys. |
| `subagents` | livekit, pipecat | `agent_transfer` in both directions between `booking_desk` and `appointment_manager`, sharing one tool set. | Derived, unverified. "I want to change an appointment I already have," then later "actually, can I also book a new one?" | A transfer to `appointment_manager` on the first ask and back to `booking_desk` on the second, with the conversation continuing rather than restarting. |
