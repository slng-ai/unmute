...
- `Temperature`/`TopP`/`TopK`/`Speed` as `FieldSpec` (added 2026-08-07, SCHEMA.md N20): the four typed generation fields Unmute owns, each declaring whether this entry's class has a slot and what the vendor calls it, per framework (rime `speed_alpha` on LiveKit but `speedAlpha` on Pipecat, sarvam `pace`, inworld `speaking_rate`). A zero FieldSpec is a hard error, same as a zero `Voice` or `Language`. `Required` and `Form` are inert on these four: a generation param is never mandatory and has no transform. This binds the *typed* fields only; a same-named key inside `params:` is still forwarded verbatim.
...
**What the catalogue never contains:** model names, voice ids, or param *values*. D10 stands: identities and author-supplied `params` keys forward verbatim, unvalidated. An entry is a code slot, not an allowlist.

The one thing it does state about params is structural, not a value judgment: whether the class accepts each of Unmute's four generation kwargs, and under what name (N20). That is the same fact `Voice.Arg` and `Language.Arg` already carry, and it governs the **typed** fields only. Every key an author writes in `params:` is forwarded untouched, including one spelled like those four, so a vendor setting Unmute has never heard of keeps working and the escape hatch stays open. Known provider brands that ride inside an aggregate route (`slng/deepgram/nova:3`) are recorded only to let the wizard present a provider once and then ask for its distributor; the route and model identity remain forwarded text.
...
- **F9: validation and generation now share one rulebook.** `Catalog.CheckVendor` (vendor known, wildcard-needs-endpoint, endpoint-has-a-slot) is called by `ir.Validate` for every listen/turn/speak/reason binding and by the drivers' `resolveService`. The hand-written `RoleProviders` map is deleted; Deepgram and ElevenLabs allowlists became call-less matrix rows (with Deepgram's own `eleven_labs`/`open_ai` spellings as aliases). A spec that validates green can no longer fail provider selection at generate time, and `unmute validate` now quotes the matrix for unknown vendors on code targets too. Leniency is deliberate: an empty vendor defers to the target default, and a role with no rows at all (Vapi) stays unrestricted per D10. **Known hole, found 2026-08-08, unfixed:** that empty-vendor leniency is the one place validate and generate still disagree. `CheckVendor` skips an unset provider, while `resolveService` sends it through `DefaultVendor` to `openai` — so a provider-less speak binding validates green and then compiles to `OpenAITTSService` holding whatever model route and voice id the author wrote for a different vendor, and on LiveKit speak (no openai row) it validates green and fails at generate instead. SCHEMA.md marks `provider` **required** on listen, speak and think, and documents no openai default anywhere, so the likely fix is that both sides reject an unset provider rather than either side inventing one. That is a schema-level call, not a catalogue one.
