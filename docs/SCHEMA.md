...
- **N38 (2026-08-13).** **`transport: cloud-websocket` on Pipecat: a route where the operator hosts nothing.** A new value in the existing `transport` field, requiring `carrier: twilio`. **No authoring field is added, removed, or renamed** anywhere: not in `agent.yaml`, not in `targets.yaml`, not in a Connection. Selecting the route is writing one value the field already accepted the shape of.
  ...
  **Every existing route compiles byte-identically.** The change is additive behind the new transport value, and no existing package's meaning moves. The telephony example set was reorganized in the same feature, which is a change of files rather than of schema: `examples/human-transfer-daily-twilio` was removed in favour of `examples/pipecat-human-transfer-twilio`, and `examples/telephony-hello`'s Pipecat target moved to this route. The `(pipecat, daily-sip, twilio)` route of N37 keeps its schema, its rows, and its code, and no longer ships a public example.
