...
- **N27 (2026-08-10).** A human transfer's **shape is a block name**, and the briefing is **free text**. `mode: cold | warm` and `briefing: summary | message | wait` are both removed; a `human_transfer` control now declares `destination` plus exactly one of `cold:` or `warm:` (section 4.7). Two separate corrections, made together because they touch the same three lines.
...
`kind: human_transfer` (rewritten 2026-08-10, N25; `destination` moved into the block 2026-08-11, N27; full design in [docs/spec/human-transfer.md](spec/human-transfer.md)):
