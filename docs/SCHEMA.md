...
- **N10.** Tool `input` is a JSON Schema object. All five targets accept JSON Schema tool inputs, so nesting is allowed here. `output` has the same shape; it was specified as enforced on code targets, but no driver implements that and it is unenforced everywhere today (N20).
...
| `output` | no | JSON Schema object | warn | Declared and carried into the compile report, but **not enforced anywhere yet** (N20): no driver reads it. Only Vapi warns, and the tag is `warn` on that basis alone. Not legal on a `builtin` tool. |
