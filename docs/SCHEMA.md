...
- **N30 (2026-08-11).** A **warm** human transfer requires `channels.phone.outbound: true`. Warm holds the caller and then *dials* the destination, so the agent places calls whatever the channel declares. A package that says `outbound: false` while emitting `calls.create` is describing a shape it does not have, and the cost is not theoretical: the `livekit-human-transfer` example shipped that way, validated green, and then refused `--to` with "no outbound direction", which reads as a driver regression rather than as the package's own declaration. Cold is unaffected; it redirects the caller's existing call and originates nothing. This tightens one cross-field rule and changes no field, no value, and no route tag.
...
- **N37 (2026-08-12).** **The Daily route gains carrier legs.** On a `daily-sip` target, `carrier:`, `connection:`, and a `channels.phone` entry are now **valid together and mutually required**: naming a carrier without a connection fails by the connection's name, a connection without a carrier fails by the carrier's name, and a phone channel still forces `capacity` with a positive `peak_starts_per_second` (§4.10). **No authoring field changes**: nothing in `agent.yaml`, `targets.yaml`, or a Connection is added, removed, or renamed. Selecting the carrier leg is combining fields the vocabulary already has.

  **This supersedes N34's clause rejecting a `channels.phone` entry on the Daily route**, and with it N34's narrowing of N30 ("the Daily route has none and cannot declare one"): on the carrier form it can, so N30 applies there in full, unnarrowed, exactly as it does on every other route with a phone channel. On the no-carrier form N34's narrowing still stands, because that form still has no channel to declare. N34 rejected the channel on the grounds that the route has no carrier connection. With a carrier the route has one, so the ground is gone. The supersession follows the official Pipecat documentation and the Daily documentation it links ([Daily SIP](https://docs.pipecat.ai/pipecat/telephony/daily-sip), [Daily + Twilio SIP](https://docs.pipecat.ai/pipecat/telephony/twilio-daily-sip), read 2026-08-12), which document `provider="daily"` SIP as connecting Daily's SIP leg to any SIP-capable carrier. It is not a preference of the feature that needed it. N34's text stays as history, unedited; its other statements (Daily SIP is the only Pipecat transport that carries a call end to end, and there is no hosting-model field) still hold.

  **The no-carrier form keeps its exact meaning.** A `daily-sip` target with no carrier is still a Daily-provisioned number, still connection-free and channel-free, still refuses both, and still compiles byte-identically. There are now two Daily forms, not one changed one.

  **The Connection key set for `(pipecat, daily-sip, twilio)` is `account_sid`, `auth_token`, `sip_address`, `from_number`.** This is the first route to mix the carrier REST vocabulary with the SIP vocabulary, and it does so because the route genuinely spans both surfaces: REST moves the inbound call the carrier just delivered into the per-call room, SIP carries every outbound leg through the operator's own trunk. `sip_username` and `sip_password` are **refused** on this route, by the same key-set gate every route already has, with the route named. They are refused rather than ignored because there is nothing on the Daily side that could carry them: all three documented Daily dial-out surfaces (daily-js, REST, daily-python) accept `sipUri` with no credential field, and the REST schema rejects unknown fields, so carrier termination on this route authenticates by IP allow-list against Daily's published static addresses (`https://ip-info.daily.co/ips/ip-info.json`, read 2026-08-12). A key that promises authentication nothing performs is worse than no key.

- **N38 (2026-08-13).** **`transport: cloud-websocket` on Pipecat: a route where the operator hosts nothing.** A new value in the existing `transport` field, requiring `carrier: twilio`. **No authoring field is added, removed, or renamed** anywhere: not in `agent.yaml`, not in `targets.yaml`, not in a Connection. Selecting the route is writing one value the field already accepted the shape of.

  **`connection:` becomes conditional on this route, and only on this route.** Everywhere else a telephony channel requires a connection. Here the platform terminates the carrier's media stream itself, so **receiving a call needs no carrier credentials at all**: a package declaring only `channels.phone.inbound` compiles with no connection, and its `.env.example` and compile report carry no carrier name. A connection becomes required the moment the package **places or redirects** a call (`channels.phone.outbound: true`, or any `human_transfer` control), and the refusal names all three keys and what each is for rather than saying "requires connection". A phone channel still forces `capacity` (§4.10), unchanged.

  **The Connection key set for `(pipecat, cloud-websocket, twilio)` is `account_sid`, `auth_token`, `from_number`.** The same three names the `(livekit, connector, twilio)` and `(pipecat, carrier-websocket, twilio)` rows use, deliberately: one `.env` drives every Twilio target in a repository. `sip_address`, `sip_username`, and `sip_password` are **refused** on this route, by the key-set gate every route already has, and the refusal now names the accepted set as well as the offending key. They are refused rather than ignored because nothing on this route speaks SIP at any point: the carrier streams audio to the platform over a WebSocket and call control is one REST request.

  **What `from_number` must hold**, stated because it is asked for and was previously left to be discovered: a voice-capable phone number the operator owns in the Twilio account those credentials belong to, **or a caller ID verified on it**. It is the caller identity the recipient sees, and when the destination is a phone number the caller identity must be a phone number too. It may be the same number that receives calls. A number has no separate "outbound" capability to enable; what can still refuse a call lives on the account, not the number: the destination country's dialling permissions, and a trial account's restriction to verified destinations (errors 21219 and 14111). Verified 2026-08-13 against Twilio's Call resource, Outgoing Caller IDs, Dialing Permissions, and error documentation.

  **One new environment name, `PIPECAT_CLOUD_ORGANIZATION`**, required exactly when the package places or redirects calls. Those compose call markup that has to name `<agent>.<organization>`, and the compiler knows the agent name and cannot know the organization. It is a platform value, not a secret, and it follows the `PIPECAT_CLOUD_*` convention already in use. Carrier names stay `<CARRIER>_*` and transfer destinations stay generic, so a second carrier on this route is new names in a connection file, not a new shape.

  **Warm transfer is refused on this route with a reason, not a denial.** A warm handoff has to act on how the destination's leg ended, which needs a callback endpoint the operator hosts, and hosting nothing is the route's entire purpose. Cold transfer works, by replacing the live call's markup at the carrier, and it carries one limit that is part of the contract rather than a defect: a **failed** transfer returns the caller to a **fresh** agent session with no memory of the call, and so does a completed transfer that ends by the destination hanging up first. Both are stated in the emitted README and in the route comparison. `source.*` call-source variables are refused on this route by name, as on `daily-sip`, because the code that fills them belongs to the carrier-websocket adapter neither route emits.

  **This supersedes N34's headline, "Pipecat telephony is the Daily route", and its clause "Cold on Pipecat is the Daily route and nowhere else."** Both were true of the code on 2026-08-12 and are not now: `cloud-websocket` carries a phone call end to end without Daily in it at all, and it carries a cold transfer through the carrier's own call control. N34's three numbered corrections still hold as written, including that the **carrier-websocket** transports have no transfer control: that was a statement about those transports and this is a different one. N34's text stays as history, unedited.

  **Every existing route compiles byte-identically.** The change is additive behind the new transport value, and no existing package's meaning moves. The telephony example set was reorganized in the same feature, which is a change of files rather than of schema: `examples/human-transfer-daily-twilio` was removed in favour of `examples/pipecat-human-transfer-twilio`, and `examples/twilio-telephony-hello`'s Pipecat target moved to this route. The `(pipecat, daily-sip, twilio)` route of N37 keeps its schema, its rows, and its code, and no longer ships a public example.

- **N39 (2026-08-13).** **The telephony example set is named after what a reader is choosing between, and one example's LiveKit target moved routes. No schema changes: not a field, not a value, not a route tag.** Four directories were renamed (`human-transfer` → `livekit-human-transfer`, `human-transfer-cloud-twilio` → `pipecat-human-transfer-twilio`, `human-transfer-daily` → `pipecat-human-transfer-daily`, `telephony-hello` → `twilio-telephony-hello`) and `twilio-telephony-hello`'s LiveKit target moved from `transport: connector` to `transport: sip`. This is recorded here because N38 recorded the previous reorganization in the same place, and because a reader with an older checkout will find their paths gone.
  **Why provider-first names.** Putting a caller through to a person is not one feature with several implementations: LiveKit refers the caller's existing SIP leg, Pipecat's Daily routes reroute the room, and `cloud-websocket` replaces the live call's markup at the carrier. Those are three mechanisms with different capabilities, so the packages are not interchangeable and their names now say which platform they belong to. The one package named after its **carrier** instead is `twilio-telephony-hello`, because it holds one target per provider and its subject is how a single carrier reaches each of them.
...
- **N42 (2026-08-14).** **Recorded exception: one environment name is written in two files.** A connection's `environment` maps a role to a variable name, and §4.12 now also requires that name in `secrets:`. The two lines answer different questions — the connection says which *role* the value plays on the route, which `secrets:` cannot express; `secrets:` says the runtime requires it, which the connection cannot express for names no author writes — but the fact is stated twice, and the only agreement between them is the §4.12 cross-check, which **warns** rather than fails.
...
| `destinations` | if any `human_transfer` is used | map of symbolic name to an env var name (N41) | core |
...
| Field | Required | Values | Tag | Notes |
|---|---|---|---|---|
| `destination` | yes | symbolic name | core | Resolves through `agent.yaml`'s top-level `destinations:` map to the name of an environment variable holding a number or SIP URI (N41). |
...
of the cross-check (amended 2026-08-14, N41). They were exempt on the grounds
...
creates is recorded as an exception in N42.
...
| ~~`transport`, `carrier`~~ | — | **Superseded by N41 (2026-08-14).** Both moved into the connection file. Writing either on a target is refused, naming the file it belongs in. |
...
| ~~`destinations`~~ | — | **Superseded by N41 (2026-08-14).** Moved to the top level of `agent.yaml` and narrowed to the UPPER_SNAKE name of an environment variable; the literal E.164 and `sip:` forms N24 accepted are refused. The per-target override is removed with nothing replacing it. Writing it on a target is refused, naming `agent.yaml`. |
...
2026-08-14, N41 — the route fields moved here from the target, and `kind:` was
removed):
