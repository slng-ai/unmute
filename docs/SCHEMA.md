...
Shape support per target: **`cold`** is LiveKit native, Vapi native, and Pipecat native on Daily SIP. Pipecat's exact Twilio, Telnyx, and Plivo carrier-WebSocket adapters also emit out-of-band carrier-REST cold transfer, but remain provisional. Deepgram is carrier-conditional in the bridge. **`warm`** (review-corrected 2026-07-15; Pipecat route re-scoped 2026-08-10) is LiveKit native, stable on Node and `beta.workflows` on Python (NOT Python-only); on Pipecat it is generated on the exact `(carrier-websocket, twilio)` route as a two-socket software bridge — the person is dialed as a second streamed call onto the same process, briefed on their own private WebSocket while the caller's own transport plays hold music, then the two sockets are bridged (not emitted yet, provisional pending TELEPHONY.md Phase 3). Daily SIP is deliberately **not** the Pipecat warm route: its shared room gives the bot one output track for every listener, so hold music and a private briefing conflict there ([TRANSFERS.md](TRANSFERS.md) section 1). On Vapi the stable `transferPlan` path needs carrier Twilio.

The two drivers finish a warm transfer with different machinery, and the schema promises the caller's experience rather than the agent's exit. On LiveKit the agent moves the person into the caller's room and shuts down, so caller and person continue alone. On Pipecat the bot owns the media path — both phone calls terminate in WebSockets on the bot process — so it stays as a silent audio bridge for the rest of the call. What both promise is that the caller stops hearing the agent and starts hearing the person.
...
| human_transfer warm (`warm:`) | native (Node stable, Python Beta) | provisional on the carrier-WebSocket **Twilio** route: a two-socket bridge, where each human has a private media socket so the briefing cannot leak to the caller ([TRANSFERS.md](TRANSFERS.md)). Telnyx and plivo fail until each has its own bridge | Twilio only (stable path) | carrier-conditional |
...
|---|---|---|
| Pipecat v1 | `models.fallback`, `thinking_audio`, `outbound` + `on_voicemail`, `mcp` tools, warm transfer on every route but carrier-WebSocket Twilio ([TRANSFERS.md](TRANSFERS.md)); transfer/task context shaping beyond the safe-core defaults — `history` other than `full`, `context.variables` subset, `include_tool_calls: false` (the workers handoff carries the running context; fine-grained shaping is not emitted yet). (`local` tools lifted 2026-07-17.) | [internal/generate/pipecat_v1.go](../internal/generate/pipecat_v1.go). Emitted: single agent, `agent_transfer` (+ `requires` guard), `tasks`, `task_groups` with `context_scope` (shared/isolated), `then` return/transfer/end, `local` tools (2026-07-17). |
