...
| human_transfer warm (`warm:`) | native (Node stable, Python Beta) on `(livekit, sip)` | not emitted yet. Daily documents warm and this project has not built it (feature 005); the carrier-WebSocket routes have no transfer control at all (N34, [TRANSFERS.md](TRANSFERS.md)) | Twilio only (stable path) | carrier-conditional |
