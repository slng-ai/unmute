...
An **orchestrator** (also called a target or a platform) is the engine that runs a voice agent: it takes in audio, runs the listen-think-speak loop, and calls out to tools. Unmute supports four of them: **LiveKit, Pipecat, Vapi, and Deepgram.**

The interesting question is not "how do we support four platforms". It is "what do we do when the four disagree". This page is the answer.

## Two kinds of target

The four split into two groups, and the split explains almost every difference you will meet.
...
**Managed target: Vapi.** The provider runs the agent for you. You do not get a project to host; you get an API with a fixed set of settings. Unmute can only use what that API already exposes. There is nowhere to put extra logic.
...
driver writes the check around its native handoff. Vapi has no
...
## One description, four outcomes

You still describe the agent only once. You do not write four versions. What changes per platform is not your description; it is the report Unmute gives you about that description.
...
1. **The four targets decide the schema.** A feature is only in Unmute's vocabulary if all four can honor what it promises, whether natively, conditionally, or through generated code. Nothing is in the schema that no platform can do.
