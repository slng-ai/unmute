...
def _slot_parts(slot_id):
    try:
        ...
        return None
    if (
        requested_date < _booking_today()
        or service not in _SERVICES
        or time not in _TIMES
    ):
        return None
...
def _demo():
    ...
    with ThreadPoolExecutor(max_workers=16) as pool:
        for suffix in range(20, 30):
            concurrent = list(
                pool.map(
                    lambda _: find_or_create_customer(f"1555010 20{suffix}"), range(16)
                )
            )
    ...
    assert (set(_state.customers), dict(_state.names)) == before, (
        "look_up_customer wrote"
    )
    ...
    assert (
        copy_two.create_booking(customer, "haircut", first_slot, confirmed=True)[
            "status"
        ]
        == "slot_unavailable"
    )
    ...
    assert (
        modify_booking(customer, booking["booking_id"], "haircut", second_slot)[
            "status"
        ]
        == "not_confirmed"
    )
    ...
    assert (
        modify_booking(
            "555 000 0000", booking["booking_id"], "haircut", second_slot, True
        )["status"]
        == "not_found"
    )
    ...
    assert (
        record_complaint("555 000 0000", "Uneven cut.")["status"]
        == "customer_not_found"
    )
    complaint = record_complaint(customer, "My cut was uneven.")
    assert complaint["status"] == "recorded"
    assert (
        _state.complaints[complaint["complaint_id"]]["customer_phone"] == "15550101010"
    )
