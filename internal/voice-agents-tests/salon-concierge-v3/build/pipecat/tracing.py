...
TRACE_NAME = "concierge" + "-" + "salon-concierge-v3-pipecat"
        ...
        def patched(span, *args, **kwargs):
            ...
            def mapped_set_attribute(key, value):
                ...
                if kind == "stt" and key == "transcript":
                    ...
                    set_attribute(
                        "langfuse.observation.metadata.character_count", value
                    )
                    set_attribute(
...
# ponytail: Pipecat 1.8.0 has no standard-tool tracing hook. Remove this
...
class TracedLLMWorker(LLMWorker):
    ...
    async def _trace_tool(self, name, arguments, call, tool_call_id=None):
        ...
        with trace.get_tracer("unmute.pipecat.tools").start_as_current_span(
            ...
            span.set_attribute(
                "langfuse.observation.input", json.dumps(arguments, default=str)
            )
            if tool_call_id:
