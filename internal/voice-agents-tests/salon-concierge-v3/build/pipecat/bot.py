"""Generated Pipecat agent for salon-concierge-v3.
...
_INPUT_TYPES = {
    "handle_complaint": {
        "problem": TypeAdapter(
            Annotated[
                str,
                Field(
                    description="What the caller is unhappy about, in their own words."
                ),
            ]
        ),
        "about": TypeAdapter(
            Annotated[
                Appointment | None,
                Field(
                    description="The appointment the complaint concerns, copied from the appointments already recorded, only when the caller means a booking this call made. A complaint about an earlier visit has none, so leave it out."
                ),
            ]
        ),
    },
    "manage_booking": {
        "action": TypeAdapter(
            Annotated[
                Literal["create", "modify", "cancel"],
                Field(
                    description="What the caller wants done to a booking. Only what they said."
                ),
            ]
        ),
        "service": TypeAdapter(
            Annotated[
                ...
                | None,
                Field(
                    description="The service, in the salon's own words, only if the caller named one. Leave it out otherwise."
                ),
            ]
        ),
        "requested_day": TypeAdapter(
            Annotated[
                str | None,
                Field(
                    description="The day in the caller's own words, like tomorrow or Friday, only if they said one."
                ),
            ]
        ),
        "requested_time": TypeAdapter(
            Annotated[
                str | None,
                Field(
                    description="The time or part of the day in the caller's own words, like 3 PM or afternoon, only if they said one."
                ),
            ]
        ),
    },
    ...
    "to_concierge": {
        "outcome": TypeAdapter(
            Annotated[
                str, Field(description="What was done for the caller, in one sentence.")
            ]
        ),
        "next_request": TypeAdapter(
            Annotated[
                str,
                Field(
                    description="What the caller now wants from the front desk, in their own words."
                ),
            ]
        ),
    },
...
_INPUT_REQUIRED = {
    "handle_complaint": {"problem"},
    "manage_booking": {"action"},
    "to_complaints": {"problem"},
    "to_concierge": {"outcome", "next_request"},
}
...
def _state_text(name, value):
    ...
    """
    if name in _STATE_STRUCTURED or name in _INPUT_NAMES:
        if value is None or value == "" or value == [] or value == {}:
            return _INPUT_EMPTY if name in _INPUT_NAMES else _STATE_EMPTY
...
def _state_lookup(state, name):
    ...
    for part in path.split("__") if path else ():
        if value is None:
            break
        value = (
            value.get(part) if isinstance(value, dict) else getattr(value, part, None)
        )
    return root, value
...
class State:
    ...
    )  # What this call booked, moved or cancelled, in the order the caller gave them. A list because a caller can do two things in one call, and the booking step appends rather than replacing, so the second does not erase the first. It starts empty: nothing recorded yet, not a decision the caller made.
    ...
    customer: Customer | None = (
        None  # Who the caller is, once the verification step has heard them agree to the number and looked the record up. Absent until then: the concierge's own instructions say to run verification before the booking step, and ordering between steps is the prompt's job rather than a code gate. No `default:` for the same reason a default was wrong on the flat status value it replaced. A default is a value the variable holds before the first word, so a defaulted customer would render in the conversation info as a real-looking record before anyone had been looked up.
    )
    ...
    customer_phone: Phone = ""  # The caller's phone number in E.164: a plus sign, then digits, with no spaces, brackets or dashes. One shape for every phone number in this package, the MANAGER_PHONE_NUMBER transfer destination included, so no prompt and no tool has to guess which shape it is holding. No `source:` here on purpose. The prefetch block below reads the carrier's fact and this variable receives it, which leaves the per-route refusal for a variable naming a source its target cannot supply exactly as strict as it is: on a route with no caller ID the entry skips and this holds its default. Offered to the caller for a yes, never acted on unasked. Somebody may be ringing from a friend's phone, or may hold a second account, so until the verification step has heard them agree this value stays unconfirmed and appears in no prompt but that step's own. An earlier version of this note said no prompt ever reads the number back. That is now false rather than merely out of date: reading it back is the whole saving, and it replaced twelve spoken digits with one yes. The read-back turn does not cache, and that trade was made deliberately.
    last_booking_day: Date | None = (
        None  # The day of the caller's most recently booked appointment, assigned from the booking step's own result so a later step or agent can read it back without sharing this call's message history.
    )
    about: Appointment | None = (
        None  # input of task handle_complaint, handoff to_complaints
    )
    action: Literal["create", "modify", "cancel"] | None = (
        None  # input of task manage_booking
    )
    next_request: str | None = None  # input of handoff to_concierge
    outcome: str | None = None  # input of handoff to_concierge
    problem: str | None = None  # input of task handle_complaint, handoff to_complaints
    requested_day: str | None = None  # input of task manage_booking
    requested_time: str | None = None  # input of task manage_booking
    service: (
...
def build_state(call_context: dict | None = None) -> State:
    ...
    if "last_booking_day" in call_start:
        setattr(state, "last_booking_day", call_start["last_booking_day"])
    if missing:
...
def _render(text: str, state, *, quote_values: bool = False) -> str:
    ...
    def _one(match: re.Match[str]) -> str:
        ...
        # str() gave it.
        value = _state_text(*_state_lookup(state, match.group(1)))
...
async def _prefetch(state, call_context) -> None:
    ...
    # Every value awaiting the caller's agreement. The emitted _refusal helper
...
{{customer__status}}. Existing means the salon already knew this customer,
...
class ComplaintSpecialistAgent(TracedLLMWorker):
    ...
    async def on_activated(self, args) -> None:
        ...
        # And a worker with a brief renders it here: the handoff wrote the brief
        # to the shared state just before activating this worker, and the prompt
        # the constructor rendered predates it.
    ...
    def build_tools(self) -> list:
        """The @tool methods, with an explicit
        schema for every step or handoff that is handed typed inputs."""
        tools = super().build_tools()
        # The framework reads a tool's schema off its Python signature, and a
        # signature cannot say what a Literal allows or what a declared class
        # holds. The handler stays the decorated method above, found by name, so
        # the framework's own call tracking still wraps it; only what the model
        # is shown changes.
        explicit = {
            "handle_complaint": FunctionSchema(
                name="handle_complaint",
                description="The verified caller is unhappy about something and wants it recorded.",
                properties={
                    "problem": _schema(_INPUT_TYPES["handle_complaint"]["problem"]),
                    "about": _schema(_INPUT_TYPES["handle_complaint"]["about"]),
                },
                required=["problem"],
            ),
            "to_concierge": FunctionSchema(
                name="to_concierge",
                description="The caller changes to a request owned by another specialist.",
                properties={
                    "outcome": _schema(_INPUT_TYPES["to_concierge"]["outcome"]),
                    "next_request": _schema(
                        _INPUT_TYPES["to_concierge"]["next_request"]
                    ),
                },
                required=["outcome", "next_request"],
            ),
        }
    ...
    async def to_concierge(
        ...
        # The brief, validated before the receiver is activated and before
        # anything is spoken: a value outside its type is refused with the field
        # named and nobody is handed over. This framework validates no tool
        # argument itself, which is why this runs on both targets.
        try:
            _inputs = _typed_inputs("to_concierge", params.arguments)
        except _StateRefused as refused:
            logger.warning("{} expects: {}", "to_concierge", refused.message)
            await params.result_callback(
                {
        ...
        # The receiver's brief: every name its prompt reads is reset, then this
        # handoff's own values are written, so it sees exactly what it was handed
        # and nothing from an earlier visit. Written before activation, because
        # on_activated renders the receiver's prompt from the shared state, and
        # left in place after: a brief stays with its worker until the next
        # handoff.
        self.state.outcome = None
        self.state.next_request = None
        for _name, _value in _inputs.items():
    ...
    async def _verify_customer_finish_verify_customer(self, args, flow_manager):
        ...
        # now and the emitted _refusal helper stops holding it back. Discarded
        # here rather than in the step's prompt: a model can be talked out of an
        # instruction, not out of a set membership test. getattr with a default:
        # this step can be reached on a path where the pre-fetch never ran, and a
        # bare read there is an AttributeError inside a finish handler.
    ...
    async def handle_complaint(
        ...
        problem: Annotated[
            str,
            Field(description="What the caller is unhappy about, in their own words."),
        ],
        about: Annotated[
            Appointment | None,
            Field(
        ...
        """The verified caller is unhappy about something and wants it recorded."""
        # What the step is handed, validated before the flow exists: refused the
        # way finish refuses a bad result, so nothing is written and the message
        # goes back to the model. This framework validates no tool argument
        # itself, which is why this runs on both targets.
        try:
            _inputs = _typed_inputs("handle_complaint", params.arguments)
        except _StateRefused as refused:
            logger.warning("{} expects: {}", "handle_complaint", refused.message)
            await params.result_callback(
                {
            ...
            return
        # Written to the shared state for this visit and put back when the step
        # finishes: the node's prompt renders from the state on the way in, and
        # an owner whose own brief shares a name gets it back.
        self._handle_complaint_previous = {
    ...
    async def _handle_complaint_finish_handle_complaint(self, args, flow_manager):
        ...
        # The visit is over: put back what the state held before it, before the
        # owner's prompt is rendered again below, so an owner whose own brief
        # shares a name reads its own value and not the step's.
        for _name, _value in self._handle_complaint_previous.items():
            setattr(self.state, _name, _value)
        # then: return — restore the owner's pre-flow context (messages and
        # tools); only the typed results cross back (merge: results, N13).
...
class ConciergeAgent(TracedLLMWorker):
    ...
    def build_tools(self) -> list:
        ...
        # is shown changes.
        explicit = {
            "manage_booking": FunctionSchema(
                name="manage_booking",
                description="The caller wants to create, modify, or cancel a booking.",
                properties={
                    "action": _schema(_INPUT_TYPES["manage_booking"]["action"]),
                    "service": _schema(_INPUT_TYPES["manage_booking"]["service"]),
                    "requested_day": _schema(
                        _INPUT_TYPES["manage_booking"]["requested_day"]
                    ),
                    "requested_time": _schema(
                        _INPUT_TYPES["manage_booking"]["requested_time"]
                    ),
                },
                required=["action"],
            ),
            "to_complaints": FunctionSchema(
                name="to_complaints",
                description="The verified caller has a complaint or service problem.",
                properties={
                    "problem": _schema(_INPUT_TYPES["to_complaints"]["problem"]),
                    "about": _schema(_INPUT_TYPES["to_complaints"]["about"]),
                },
                required=["problem"],
            ),
        }
    ...
    async def to_complaints(
        ...
        # argument itself, which is why this runs on both targets.
        try:
            _inputs = _typed_inputs("to_complaints", params.arguments)
        except _StateRefused as refused:
            logger.warning("{} expects: {}", "to_complaints", refused.message)
            await params.result_callback(
                {
                    "refused": f"Not started: {refused.message}. Ask the caller, then hand over again with a value that fits."
                }
    ...
    async def manage_booking(
        ...
        requested_day: Annotated[
            str | None,
            Field(
                description="The day in the caller's own words, like tomorrow or Friday, only if they said one."
            ),
        ] = None,
        requested_time: Annotated[
            str | None,
            Field(
                description="The time or part of the day in the caller's own words, like 3 PM or afternoon, only if they said one."
            ),
        ...
        """The caller wants to create, modify, or cancel a booking."""
        # What the step is handed, validated before the flow exists: refused the
        # way finish refuses a bad result, so nothing is written and the message
        # goes back to the model. This framework validates no tool argument
        # itself, which is why this runs on both targets.
        try:
            _inputs = _typed_inputs("manage_booking", params.arguments)
        except _StateRefused as refused:
            logger.warning("{} expects: {}", "manage_booking", refused.message)
            await params.result_callback(
                {
                    "refused": f"Not started: {refused.message}. Ask the caller, then run the step again with a value that fits."
                }
        ...
        # finishes: the node's prompt renders from the state on the way in, and
        # an owner whose own brief shares a name gets it back.
        self._manage_booking_previous = {
            "action": self.state.action,
            "service": self.state.service,
            "requested_day": self.state.requested_day,
            "requested_time": self.state.requested_time,
        }
        for _name, _value in _inputs.items():
            setattr(self.state, _name, _value)
    ...
    async def _manage_booking_transfer_manage_booking_to_complaints(
        ...
        # The brief, validated before anything moves: a refusal leaves the step
        # active and the message goes back to the model.
        try:
            _inputs = _typed_inputs("to_complaints", dict(args))
        except _StateRefused as refused:
            logger.warning("{} expects: {}", "to_complaints", refused.message)
            return {
                "refused": f"Not started: {refused.message}. Ask the caller, then hand over again with a value that fits."
        ...
        try:
            ...
            self.context.set_tools(tools)
            # The receiver's brief: reset, then written, before activation, so its
            # prompt renders exactly what it was handed. Nothing of this step's
            # own inputs is put back on this path: the caller is leaving with the
            # brief, and a name the two share would be erased by a restore.
            self.state.problem = None
            self.state.about = None
            for _name, _value in _inputs.items():
                setattr(self.state, _name, _value)
    ...
    async def _manage_booking_complete_manage_booking(self):
        ...
        # owner's prompt is rendered again below, so an owner whose own brief
        # shares a name reads its own value and not the step's.
        for _name, _value in self._manage_booking_previous.items():
            setattr(self.state, _name, _value)
        # then: return — restore the owner's pre-flow context (messages and
        # tools); only the typed results cross back (merge: results, N13).
        messages, tools = self._manage_booking_snapshot
        ...
        self.state.last_booking_day = (
            self._manage_booking_results["manage_booking"].get("appointment") or {}
        ).get("scheduled_date")
        return {"status": "ok"}, None
...
async def _flow_tool_cancel_booking(args, flow_manager, state=None):
    ...
    refusal = _refusal(
        "cancel_booking", state, [("customer_phone", "run verify_customer first.")]
    )
    if refusal:
...
async def _flow_tool_create_booking(args, flow_manager, state=None):
    ...
    refusal = _refusal(
        "create_booking", state, [("customer_phone", "run verify_customer first.")]
    )
    if refusal:
        return {"refused": refusal}
    # Cover the wait, same contract as _direct_tool: TTSSpeakFrame is a
...
async def _flow_tool_list_bookings(args, flow_manager, state=None):
    ...
    refusal = _refusal(
        "list_bookings", state, [("customer_phone", "run verify_customer first.")]
    )
    if refusal:
        return {"refused": refusal}
    # Cover the wait, same contract as _direct_tool: TTSSpeakFrame is a
...
async def _flow_tool_modify_booking(args, flow_manager, state=None):
    ...
    refusal = _refusal(
        "modify_booking", state, [("customer_phone", "run verify_customer first.")]
    )
    if refusal:
        return {"refused": refusal}
    # Cover the wait, same contract as _direct_tool: TTSSpeakFrame is a
...
async def _flow_tool_record_complaint(args, flow_manager, state=None):
    ...
    refusal = _refusal(
        "record_complaint", state, [("customer_phone", "run verify_customer first.")]
    )
    if refusal:
        return {"refused": refusal}
    # Cover the wait, same contract as _direct_tool: TTSSpeakFrame is a
