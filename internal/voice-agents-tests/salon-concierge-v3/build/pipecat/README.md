...
handoff refused for a bad brief never speaks it. Pipecat waits for that speech
to stop before it activates the receiving worker. Without `announce`, the
...
The values are validated before the step is entered or the receiver is
activated. A value outside its type is refused with the field named, nothing is
started, and the message reaches the agent that supplied it, which can correct
itself or ask the caller. A required value the agent left out is refused the
same way. An optional value left out renders as `not given.`,
and the step may ask for that one thing.
...
pipecat cloud secrets set salon-concierge-v3-pipecat-secrets --file .env --region eu-central
pipecat cloud deploy
pipecat cloud agent status salon-concierge-v3-pipecat
```
...
**salon-concierge-v3-pipecat** is `name:` in your `agent.yaml` joined to the `pipecat`
...
delete it with `pipecat cloud agent delete salon-concierge-v3-pipecat` and
...
```sh
pipecat cloud agent logs salon-concierge-v3-pipecat -n 250
pipecat cloud agent logs salon-concierge-v3-pipecat -s <session-id>
```
...
```sh
COLUMNS=100000 pipecat cloud --output json agent logs salon-concierge-v3-pipecat -n 2000 -s <session-id>
```

`pipecat cloud agent sessions salon-concierge-v3-pipecat` lists the sessions themselves, which is
...
```sh
pipecat cloud secrets set salon-concierge-v3-pipecat-<region>-secrets --file .env --region <region>
pipecat cloud deploy salon-concierge-v3-pipecat-<region> --region <region> --secrets salon-concierge-v3-pipecat-<region>-secrets
```
   ...
   <Response>
     ...
     <Connect>
       <Stream url="wss://eu-central.api.pipecat.daily.co/ws/twilio">
         <Parameter name="_pipecatCloudServiceHost" value="salon-concierge-v3-pipecat.YOUR_ORGANIZATION"/>
       </Stream>
     </Connect>
   </Response>
   ```
...
- **Agent names are globally unique across regions**, so `salon-concierge-v3-pipecat` cannot exist
...
can start a session, then, is knowing the `salon-concierge-v3-pipecat.<your organization>`
...
One log, and only one: `pipecat cloud agent logs salon-concierge-v3-pipecat`. There is no
...
`concierge-salon-concierge-v3-pipecat`; one trace contains the full conversation. The
