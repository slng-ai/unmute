...
logger = logging.getLogger("salon-concierge-v3")
...
When this step is complete, call `finish` with: complaint, summary.
...
When this step is complete, call `finish` with: appointment, summary.
...
def _render(text: str, userdata, *, quote_values: bool = False) -> str:
    ...
    def _one(match: re.Match[str]) -> str:
        ...
        # str() gave it.
        value = _state_text(*_state_lookup(userdata, match.group(1)))
...
def _dispatched_call_start(metadata: dict | None = None) -> dict:
    ...
    if "last_booking_day" in values:
        value = values["last_booking_day"]
        if not (isinstance(value, str)):
            raise RuntimeError("call_start.last_booking_day must be string")
    if missing:
...
def _hydrate_call_start(userdata, values: dict) -> None:
    ...
    if "last_booking_day" in values:
        userdata.last_booking_day = values["last_booking_day"]
    return None
...
class ComplaintSpecialist(Agent):
    ...
    async def to_concierge(
        ...
        # The brief, validated before the receiver exists and before anything is
        # spoken: a value outside its type is refused with the field named and
        # nobody is handed over.
        try:
            _inputs = _typed_inputs(
                "to_concierge", {"outcome": outcome, "next_request": next_request}
            )
        except _StateRefused as refused:
            logger.warning("%s expects: %s", "to_concierge", refused.message)
            return {
        ...
        # and nothing from an earlier visit. Written before the receiver exists,
        # because its prompt renders on entry from the shared state, and left in
        # place after: a brief stays with its agent until the next handoff.
        ctx.userdata.outcome = None
        ctx.userdata.next_request = None
    ...
    async def verify_customer(self, ctx: RunContext) -> dict:
        ...
        # now and the emitted _refusal helper stops holding it back. Discarding
        # here rather than in the step's prompt is the point: a model can be
        # talked out of an instruction, not out of a set membership test.
        #
        # getattr with a default: the set is created by the pre-fetch, and this
        # step can be reached on a path where the pre-fetch never ran. A bare
        # attribute read there is an AttributeError inside a finish handler,
        # which is a call that dies mid-step.
    ...
    async def handle_complaint(
        ...
        # What the step is handed, validated before the step exists: refused the
        # way finish refuses a bad result, so nothing is written and the message
        # goes back to the model, which can correct itself or ask the caller.
        try:
            _inputs = _typed_inputs(
                "handle_complaint", {"problem": problem, "about": about}
            )
        except _StateRefused as refused:
            logger.warning("%s expects: %s", "handle_complaint", refused.message)
            return {
                "refused": f"Not started: {refused.message}. Ask the caller, then run the step again with a value that fits."
            }
        # Written to the shared state for this visit and put back after it. The
        # step's prompt and the router's template variables both read the state
        # object and nothing else, and an owner whose own brief shares a name
        # gets it back rather than losing it to a clear.
        _previous = {"problem": ctx.userdata.problem, "about": ctx.userdata.about}
        for _name, _value in _inputs.items():
            setattr(ctx.userdata, _name, _value)
        # N13: snapshot before the task, restore after. An awaited AgentTask
        # merges its own turns into this agent's context when it returns
        ...
        # nothing, 2026-08-15). The task-group branch below always did this.
        owner_ctx = self.chat_ctx.copy()
        result = await HandleComplaint()  # history: reset — the task starts fresh
        # The visit is over: put back what the state held before it. Not in a
        # finally, on purpose: on the handoff path above the receiver's brief
        # was written on the way out, and a name it shares with this step's
        # inputs would be erased by a restore that ran regardless.
...
class Concierge(Agent):
    ...
    async def to_complaints(
        ...
        try:
            _inputs = _typed_inputs(
                "to_complaints", {"problem": problem, "about": about}
            )
        except _StateRefused as refused:
            logger.warning("%s expects: %s", "to_complaints", refused.message)
            return {
                "refused": f"Not started: {refused.message}. Ask the caller, then hand over again with a value that fits."
            }
        # The receiver's brief: every name its prompt reads is reset, then this
        ...
        # because its prompt renders on entry from the shared state, and left in
        # place after: a brief stays with its agent until the next handoff.
        ctx.userdata.problem = None
        ctx.userdata.about = None
        for _name, _value in _inputs.items():
            setattr(ctx.userdata, _name, _value)
        # history: reset — the target starts fresh (a handoff marker still lands).
    ...
    async def manage_booking(
        ...
        try:
            ...
            logger.warning("%s expects: %s", "manage_booking", refused.message)
            return {
                "refused": f"Not started: {refused.message}. Ask the caller, then run the step again with a value that fits."
            }
        # Written to the shared state for this visit and put back after it. The
        ...
        # gets it back rather than losing it to a clear.
        _previous = {
            "action": ctx.userdata.action,
            "service": ctx.userdata.service,
            "requested_day": ctx.userdata.requested_day,
            "requested_time": ctx.userdata.requested_time,
        }
        for _name, _value in _inputs.items():
            setattr(ctx.userdata, _name, _value)
        # N13: snapshot before the task, restore after. An awaited AgentTask
        ...
        owner_ctx = self.chat_ctx.copy()
        try:
            result = await ManageBooking()  # history: reset — the task starts fresh
        ...
        ctx.userdata.last_booking_day = (result.get("appointment") or {}).get(
            "scheduled_date"
        )
        # The values above are in this agent's own prompt, which was rendered on
        # entry and would otherwise still read as it did before this step ran.
...
class HandleComplaint(_RetryEmptyTaskResponseMixin, AgentTask[dict]):
    ...
    async def record_complaint(
        ...
        refusal = _refusal(
            "record_complaint",
            ctx.userdata,
            [("customer_phone", "run verify_customer first.")],
        )
        if refusal:
    ...
    async def finish(
        ...
        try:
            _values = _typed_result(
                "handle_complaint", {"complaint": complaint, "summary": summary}
            )
        except _StateRefused as refused:
...
class ManageBooking(_RetryEmptyTaskResponseMixin, AgentTask[dict]):
    ...
    async def check_availability(
        ...
        service: Literal["haircut", "haircolor", "haircut_and_haircolor", "dry_cut"],
    ) -> dict:
    ...
    async def to_complaints(
        ...
        if not self._claim_terminal():
            return
        # The brief, validated before the receiver exists. A refusal gives the
        # terminal claim back, so the step can still finish or hand over once
        # the model has corrected itself.
        try:
            _inputs = _typed_inputs(
                "to_complaints", {"problem": problem, "about": about}
            )
        ...
        try:
            # The receiver's brief: reset, then written, before the receiver
            # exists, so its prompt renders exactly what it was handed.
            ctx.userdata.problem = None
            ctx.userdata.about = None
            for _name, _value in _inputs.items():
                setattr(ctx.userdata, _name, _value)
            # history: reset — the target starts fresh.
            self.complete(_TaskTransfer(ComplaintSpecialist()))
    ...
    async def finish(
        ...
        try:
            _values = _typed_result(
                "manage_booking", {"appointment": appointment, "summary": summary}
            )
        except _StateRefused as refused:
...
@server.rtc_session(agent_name="salon-concierge-v3-livekit")
async def entrypoint(ctx: JobContext) -> None:
    ...
    setup_langfuse(
        ...
        metadata={
            ...
            "langfuse.trace.name": "concierge" + "-" + "salon-concierge-v3-livekit",
        },
    )
