...
| an `.env.example` naming every required variable | the values, from their secret store |
...
`.env.example` is the exact list for that build:
...
Give the user that line and let them decide, rather than deciding for them.
