...
var versionPins = []struct {
	...
	//
	// A livekit plugin distribution counts, because those version in lockstep
	// with livekit-agents: the 1.8.1 install carries livekit-plugins-openai,
	// -silero, -deepgram, -cartesia and -slng all at 1.8.1. So a page naming a
	// plugin version is naming the framework version, and can rot the same way.
	// `livekit-plugins-openai` 1.6.10 sat in two pages describing what that
	// version injects by itself, and the bump left both behind: the framework
	// name alone did not match them, so this gate read clean while two pages
	// named a version the compiler refuses.
	//
	// Pipecat's plugins are deliberately NOT here: pipecat-slng versions on its
	// own line (0.5.2 against pipecat-ai 1.10.0), so holding one to the
	// framework ceiling would refuse a page that is right.
	{"a stated pin", regexp.MustCompile("`(?:pipecat-ai|livekit-agents|livekit-plugins-[a-z0-9-]+)`" + `\s+(\d+\.\d+\.\d+)`)},
}
