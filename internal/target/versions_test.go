...
func TestCheckVersionAgainstWindow(t *testing.T) {
	cases := []struct {
		...
		{name: "pipecat exact", provider: Pipecat, version: "1.9.0"},
		...
		{name: "above ceiling names the fix", provider: Pipecat, version: "1.10.0", wantErr: "a newer unmute may support it"},

		{name: "below pipecat floor", provider: Pipecat, version: "1.6.9", wantErr: "outside the supported range (exactly 1.9.0)"},
