...
type TelephonyRoute struct {
	...
	// carried into compile-report.json so a deploy tool or a runbook can name
	// the exact endpoint rather than guessing it.
...
func TelephonyRoutes() map[TelephonyKey]TelephonyRoute {
	...
	// No AutoWebhookEndpoint: the number points at a TwiML Bin, which is a
	// console object rather than a URL, so there is nothing for anything to
	// write. Nothing this route emits publishes an endpoint of its own either,
	// which is why none appears above.
	route.ManualSteps = []string{
