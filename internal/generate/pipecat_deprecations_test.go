...
// TestEmittedRealtimeAvoidsTheDeprecatedKwargs is the same question put to the
// realtime service, and it is the one construction in this driver where the
// deprecated shape is the one a reader of the vendor's own examples would write.
//
// pipecat-ai 1.10.0 still accepts `model=` and `session_properties=` as
// top-level constructor arguments (services/openai/realtime/llm.py:236-247) and
// deprecates both for removal in 2.0.0 (llm.py:255-270). Each is warned about
// once per service (llm.py:308 and llm.py:312), so a worker would print it at
// every start, and the author cannot fix it because the call is generated. Both
// belong inside `settings=`, where the service keeps them in sync with the
// session properties itself (llm.py:113-126).
//
// Asserted on the construction alone rather than the module: `model=` appears in
...
func TestEmittedRealtimeAvoidsTheDeprecatedKwargs(t *testing.T) {
	...
	// The constructor's own arguments are the lines indented by exactly eight
	// spaces; anything deeper is inside settings= and is where these two belong.
	for _, line := range strings.Split(call, "\n") {
		if !strings.HasPrefix(line, "        ") || strings.HasPrefix(line, "         ") {
			continue
		}
		for _, deprecated := range []string{"model=", "session_properties="} {
			if strings.HasPrefix(strings.TrimSpace(line), deprecated) {
				t.Errorf("the realtime construction passes %s as a top-level argument, which pipecat 1.10.0 deprecates and warns about on every worker start; move it inside settings=:\n%s", deprecated, line)
			}
		}
	}
	if !strings.Contains(call, "settings=OpenAIRealtimeLLMService.Settings(") {
		t.Errorf("the realtime service is not constructed with settings=:\n%s", call)
	}
}
