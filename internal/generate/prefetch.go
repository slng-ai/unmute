...
func Prefetch(agent *ir.Agent, stateExpr string, request func(entry ir.Prefetch) PrefetchRequest) (PrefetchBlock, bool) {
	...
	block := PrefetchBlock{NeedsAsync: true}
...
async def _prefetch_wait(work, deadline):
    task = asyncio.create_task(work)
    task.add_done_callback(_prefetch_outcome)
    try:
...
`)
	for _, entry := range agent.Prefetch {
		...
		switch {
		case entry.Clock != "":
			writePrefetchClock(&body, agent, entry)
		case entry.Source != "":
			writePrefetchSource(&body, agent, entry)
		default:
			writePrefetchTool(&body, agent, entry, stateExpr, request(entry))
		}
		for _, line := range strings.Split(strings.TrimSuffix(body.String(), "\n"), "\n") {
			b.WriteString("    " + line + "\n")
		}
		fmt.Fprintf(&b, "    except TimeoutError:\n        logger.warning(\"prefetch %s: startup budget expired; %s\")\n        return\n", entry.Name, prefetchKeepsDefault(entry))
		fmt.Fprintf(&b, "    except Exception:\n        logger.exception(\"prefetch %s: failed; %s\")\n", entry.Name, prefetchKeepsDefault(entry))
	}
...
func writePrefetchClock(b *strings.Builder, agent *ir.Agent, entry ir.Prefetch) {
	...
	values := make([]string, 0, len(entry.Assign))
	for _, pair := range entry.Assign {
		field := strings.TrimPrefix(pair.Value.(string), "result.")
		values = append(values, pyQuote(pair.Key)+": _prefetch_bounded("+pyQuote(pair.Key)+", "+prefetchClockExpr(field, entry.Timezone)+")")
	}
	writePrefetchSave(b, agent, entry, values, "    ")
}

func writePrefetchSave(b *strings.Builder, agent *ir.Agent, entry ir.Prefetch, values []string, indent string) {
	inputs := make([]string, 0, len(entry.Inputs))
...
func writePrefetchSource(b *strings.Builder, agent *ir.Agent, entry ir.Prefetch) {
	fmt.Fprintf(b, "    _value = (call_context or {}).get(%s)\n", pyQuote(string(entry.Source)))
	if prefetchSourceIsANumber(entry.Source) {
		fmt.Fprintf(b, "    _value = _prefetch_number(%s, _value)\n", pyQuote(string(entry.Source)))
	}
	b.WriteString("    if _value in (None, \"\"):\n")
	fmt.Fprintf(b, "        logger.info(\"prefetch %s: skipped, the call carries no %s\")\n", entry.Name, entry.Source)
	b.WriteString("    else:\n")
	values := make([]string, 0, len(entry.Assign))
	for _, pair := range entry.Assign {
		values = append(values, pyQuote(pair.Key)+": _prefetch_bounded("+pyQuote(pair.Key)+", _value)")
	}
	writePrefetchSave(b, agent, entry, values, "        ")
}
...
func writePrefetchTool(b *strings.Builder, agent *ir.Agent, entry ir.Prefetch, stateExpr string, request PrefetchRequest) {
	...
	indent := "    "
	if len(guards) > 0 {
		fmt.Fprintf(b, "    if %s:\n        logger.info(\"prefetch %s: skipped, an input is unavailable\")\n    else:\n", strings.Join(guards, " or "), entry.Name)
		indent = "        "
	}
	fmt.Fprintf(b, "%sasync def _lookup():\n", indent)
	if request.Local {
		fmt.Fprintf(b, "%s    handler = tools.%s.%s\n", indent, request.Name, request.Name)
		fmt.Fprintf(b, "%s    if inspect.iscoroutinefunction(handler):\n%s        return await handler(%s)\n", indent, indent, request.CallKwargs)
		fmt.Fprintf(b, "%s    result = await asyncio.to_thread(handler, %s)\n", indent, request.CallKwargs)
		fmt.Fprintf(b, "%s    return await result if inspect.isawaitable(result) else result\n", indent)
	} else {
		...
		fmt.Fprintf(b, "%s        response = await client.post(%s, %sjson=%s, timeout=max(0.001, deadline - asyncio.get_running_loop().time()))\n", indent, request.URLExpr, auth, request.BodyExpr)
		fmt.Fprintf(b, "%s        response.raise_for_status()\n%s        return response.json()\n", indent, indent)
	}
	fmt.Fprintf(b, "%sresult = await _prefetch_wait(_lookup(), deadline)\n", indent)
	values := make([]string, 0, len(entry.Assign))
	for _, pair := range entry.Assign {
		field := strings.TrimPrefix(pair.Value.(string), "result.")
		values = append(values, pyQuote(pair.Key)+": _prefetch_bounded("+pyQuote(pair.Key)+", result.get("+pyQuote(field)+"))")
	}
	writePrefetchSave(b, agent, entry, values, indent)
}
