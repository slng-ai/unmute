...
// TestLiveEmitsOneLiveServiceAndNothingItReplaces reads the emitted bot for
// the live_model fixture: one live service configured with the prompt, the
...
func TestLiveEmitsOneLiveServiceAndNothingItReplaces(t *testing.T) {
	...
	if strings.Contains(readme, "## Turn taking") {
		t.Error("README.md explains turn taking, which no setting of a live package decides")
	}
	report := artifactFile(t, artifact, "compile-report.json")
	if !strings.Contains(report, "live: openai via OpenAILiveLLMService") {
		t.Errorf("compile-report.json does not name the live service:\n%s", report)
	}
}

// TestLiveWithoutABackendRunsAlone: no think entry means no delegation and
// no Responses import, and the runbook says the model declines tool work.
func TestLiveWithoutABackendRunsAlone(t *testing.T) {
	...
	if readme := artifactFile(t, artifact, "README.md"); !strings.Contains(readme, "No backend is named") {
