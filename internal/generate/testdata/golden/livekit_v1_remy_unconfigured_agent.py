...
class Events(Agent):
    ...
    async def do_event(self, ctx: RunContext) -> dict:
        """The caller is ready to plan their event; run the events flow."""
        group = TaskGroup(
...
class Reservations(Agent):
    ...
    async def do_reserve(self, ctx: RunContext) -> dict:
        """The caller is ready to book a table; run the reservation flow."""
        group = TaskGroup(
            chat_ctx=self.chat_ctx.copy(exclude_instructions=True),
            summarize_chat_ctx=False,
        )
...
class ConfirmBooking(AgentTask[dict]):
    ...
    @function_tool
    async def send_confirmation(self, ctx: RunContext, name: str, phone: str, summary: str) -> dict:
    ...
    @function_tool
    async def finish(self, ctx: RunContext, sent: bool) -> str:
...
class FindSlot(AgentTask[dict]):
    ...
    @function_tool
    async def finish(self, ctx: RunContext, date: str, party_size: int, time: str) -> str:
        """Record the result of this step and finish."""
...
class QualifyEvent(AgentTask[dict]):
    ...
    @function_tool
    async def finish(self, ctx: RunContext, date: str, headcount: int, occasion: str) -> str:
        """Record the result of this step and finish."""
