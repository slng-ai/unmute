...
func ForwardDeclaration(requires []string, suppliers map[string]string) string {
	if len(requires) == 0 {
		return ""
	}
...
// attribute lookups.
func guardHelperSource(suppliers map[string]string) string {
...
func PrerequisiteGuard(agent *ir.Agent) (string, bool) {
	...
	return guardHelperSource(SupplierIndex(agent.Controls)), true
}
...
func delegateForwardDeclaration(agent *ir.Agent, c *ir.Delegate) string {
	return ForwardDeclaration(c.Requires, SupplierIndex(agent.Controls))
}
