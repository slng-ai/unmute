...
// attribute lookups.
func guardHelperSource(suppliers map[string]string, unconfirmed bool, lists []string, paths bool) string {
...
`)
	...
	b.WriteString("}\n\n")
	switch {
	case len(lists) > 0 || paths:
		// The fuller predicate, emitted only for a package that declares a list
		// or names a path. Everything else keeps one of the two shapes below,
		// byte for byte.
		b.WriteString("# Every name declared as a list. An empty one means nothing has been\n" +
...
def _unmet_prerequisites(state, names):
    ...
    for name in names:
        ...
        for step in path.split(".") if path else ():
            if value is None:
                break
            value = value.get(step) if isinstance(value, dict) else getattr(value, step, None)
...
func PrerequisiteGuard(agent *ir.Agent) (string, bool) {
	...
	return guardHelperSource(SupplierIndex(agent.Controls), PrefetchUnconfirmed(agent),
		declaredLists(agent), guardsNameAPath(agent)), true
}

// declaredLists is every value declared as a list, sorted, which is the set the
// emitted predicate treats an empty value of as unmet.
func declaredLists(agent *ir.Agent) []string {
	var out []string
...
// guardsNameAPath reports whether any guard names a field inside a declared
// shape, which is what the walking predicate is for.
func guardsNameAPath(agent *ir.Agent) bool {
	for _, control := range agent.Controls {
		var requires []string
		switch control := control.(type) {
		case *ir.Delegate:
