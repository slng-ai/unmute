...
```text
compiled examples/simple-prompt/build/pipecat
starting the local Pipecat agent...

  ▸ http://localhost:8765/?agent=pipecat
    ctrl-c to stop  ·  logs: examples/simple-prompt/build/pipecat/dev.log
```
...
```sh
unmute dev examples/simple-prompt --target pipecat --port 8790 --bot-port 7890
```
...
```sh
unmute dev examples/simple-prompt --target pipecat --verbose
```

## Not opening the browser

```sh
unmute dev examples/simple-prompt --target pipecat --no-open
```
...
LIVEKIT_HOST_PORT=7890 LIVEKIT_TCP_HOST_PORT=7891 LIVEKIT_UDP_HOST_PORT=7892 \
  unmute dev examples/subagents --target livekit --port 8766
```
