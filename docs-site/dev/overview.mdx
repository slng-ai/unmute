...
Controls do not get a row. A delegate or a transfer reaches the framework as a
function call, so it would otherwise be timed like a tool, and a delegate does not
return until the flow it started has finished. A three-turn task would appear as
one very slow tool, which is the opposite of what the numbers are for.

[Reading the latency numbers](/dev/latency) covers what each number means, where
