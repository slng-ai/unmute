...
| `unmute dev --telephony` | the target's phone route | Docker, or `uv` on Pipecat `cloud-websocket`; no carrier account |

Which plane each route gets, what a local call proves, and what it cannot tell
you are on [call your agent locally](/dev/local-telephony).

## Where to go next

<Columns cols={2}>
  <Card title="Call your agent locally" icon="phone" href="/dev/local-telephony">
    One command, a real phone codec, and no carrier account.
  </Card>
