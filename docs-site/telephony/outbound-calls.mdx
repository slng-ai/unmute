...
route reads a destination number for it, and the compile report never marks
an outbound feature on it. [Read the report fields](/reference/cli/compile#the-report).
...
<Columns cols={2}>
  <Card title="Deploy to LiveKit Cloud" icon="cloud" href="/deploy/livekit-cloud">
    Take the `sip` or `connector` route live.
  </Card>
  <Card title="Deploy to Pipecat Cloud" icon="cloud" href="/deploy/pipecat-cloud">
    Take the `cloud-websocket` or `daily-sip` route live.
  </Card>
