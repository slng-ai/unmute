...
variable keeps its default, rather than the package being refused.
[Where it works](/build/prefetch#where-it-works) covers that shape, under
"The number an outbound call carries", and what a `cloud-websocket` call has to
carry for it to arrive.
