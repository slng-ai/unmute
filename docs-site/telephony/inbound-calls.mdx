...
set, then `pipecat cloud deploy`. Declare
[`warm_instances: 1`](/reference/targets-yaml#instances-held-ready) on the target
first, or the first call can arrive before the container is listening. The generated
`build/pipecat/README.md` prints the exact TwiML Bin markup, with your region and
agent name already filled in; paste it into a TwiML Bin in the Twilio Console and
point the number's voice configuration at it. [Pipecat over
Twilio](/telephony/pipecat-twilio) is that route end to end: what the markup
carries, why the warm instance is what makes the call answerable, and the three
ways it goes quiet.
...
## Later, when you redeploy

An ordinary redeploy leaves all of this alone: the trunk, the rule, and the
number keep working. Renaming the agent does not, because the dispatch rule
names the old worker as a string and no deploy updates it. The number then rings
and nothing answers, while the agent reports healthy — it was never dispatched.
[Renaming the agent breaks that rule](/deploy/livekit-cloud#renaming-the-agent-breaks-that-rule)
walks the swap; [changing the agent](/deploy/going-live#except-changing-its-name)
lists the equivalent record on every other route.
...
<AccordionGroup>
  ...
  <Accordion title="the deployed agent never answers">
    Check `pipecat cloud agent status <agent-name>` or `lk agent status` first: a
