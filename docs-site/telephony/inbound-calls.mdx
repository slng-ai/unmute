...
For `livekit`, follow [LiveKit Cloud](/deploy/livekit-cloud): `lk agent
create`, then run `bash telephony-setup.sh` from `build/livekit` to create
the inbound trunk and dispatch rule. Then do the [Twilio Console
steps](/telephony/twilio#set-up-the-livekit-sip-trunk-in-twilio-console) to
attach your number to the trunk.
...
<AccordionGroup>
  ...
  <Accordion title="the call rings and drops with no agent, on livekit">
    ...
    and does `lk agent status` show the agent up.
  </Accordion>
</AccordionGroup>
