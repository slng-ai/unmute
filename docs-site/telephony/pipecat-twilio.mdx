...
description: "A caller dials your Twilio number and your agent picks up. This is the three steps that make that happen, in the order you do them."
sidebarTitle: "Pipecat route"
---

This is the `cloud-websocket` transport with the `twilio` carrier. Your number
points at a small piece of markup in the Twilio console, and that markup streams
...
This route is **not SIP**. There is no trunk, no webhook of yours, no tunnel, and
no public URL. It shares nothing with the LiveKit Twilio route except the account
your number lives in.
...
<Note>
  ...
  is the same route with the reasons attached. Read it before the first call
  rather than after it.
</Note>

On this page:

- [Before you start](#before-you-start) - the accounts, the CLIs, and the one value you look up
- [1. Write the route into the package](#1-write-the-route-into-the-package) - the connection and the target, key by key
- [2. Deploy with a warm instance](#2-deploy-with-a-warm-instance) - the setting that makes the call answerable
- [3. Point the number at your agent](#3-point-the-number-at-your-agent) - the TwiML Bin, and the number's own page
- [Call the number](#call-the-number) - the real call
- [The values that have to agree](#the-values-that-have-to-agree) - region, service host, and the number
...
- [Why this route has no token](#why-this-route-has-no-token) - what a static Bin cannot do
- [Where to go next](#where-to-go-next)

## Before you start

You need a Twilio account with a voice-capable number, a
[Pipecat Cloud](/deploy/pipecat-cloud) account, and the `pipecat` CLI. The
Twilio CLI is optional, and it is the quickest way to check the number's state.

Then look up one value, your organisation **slug**:
...
This is the one value the compiler cannot know, and getting it wrong is the most
common way this route fails.
...
<ParamField path="warm_instances" type="integer">
  ...
  the call answerable: see [the next step](#2-deploy-with-a-warm-instance).
</ParamField>
...
so the session expires before the container is ready and the call is never picked
up at all. A knowledge base makes this more likely, because the corpus is
...
[Pipecat Cloud](/deploy/pipecat-cloud) walks the deploy and the secret set in
full.

## 3. Point the number at your agent

Two console objects, in this order: make the Bin, then tell the number to use it.

<Steps>
  <Step title="Create the TwiML Bin">
    In the Twilio Console, go to **TwiML**, **TwiML Bins**, then the plus button.
    Name it anything. Paste the markup from your own `build/pipecat/README.md`,
    where the address and the agent name are already correct:
    ...
    `YOUR_ORGANIZATION` is the slug you looked up. It is the only value you fill
    in by hand. Save the Bin.

    The `<Say>` line is a cold start cushion, not a requirement. Starting this
    agent from cold takes a few seconds, and a caller who hears nothing hangs up.
    Drop the line once you keep a warm instance.
  </Step>

  <Step title="Point the number at the Bin">
    Now go to **Phone Numbers**, **Manage**, **Active Numbers**, and open your
    number. Under **Voice Configuration**, find "A call comes in", choose
    **TwiML Bin** from the first dropdown, and pick the Bin you just made from
    the second. Save.

    Making the Bin does not route anything on its own. This step is what connects
    the two, and it is the one people skip.
  </Step>

  <Step title="Check the number is free">
    A number attached to a SIP trunk ignores its Voice configuration, silently,
    so the Bin is never consulted. Read the state back rather than testing by
    ear:
    ...
    An empty `trunkSid` is what you want. One number serves one route at a time,
    so [buy a second number](/telephony/twilio#give-each-route-its-own-number) if
    you want the LiveKit route live as well.
  </Step>
</Steps>

## Call the number

Call it from a phone. You hear the `<Say>` line, then the agent greets you.
Speak, and it replies.

That call is the only proof of the phone path. [`unmute dev`](/dev/overview) is a
browser session, and it stops exactly where the phone leg starts.

## The values that have to agree

Three things have to line up. When one does not, the failures look nothing like
each other.
...
target. One declaration lands in three places, and the platform needs all three
to match:
...
that one line, compile again, and paste the new address into the Bin. Pipecat's
[regions guide](https://docs.pipecat.ai/pipecat-cloud/guides/regions) says what
...
filled in for you; the slug is the value from
[Before you start](#before-you-start).

A service host the platform refuses is rejected before it reaches your agent, so
**your agent's log stays completely empty**. The caller hears the spoken line and
then silence. Pipecat's own [Twilio websocket
...
Adding the caller's number or the called number here is a common suggestion, and
it does nothing: no module in this package reads it.
...
[`warm_instances`](#2-deploy-with-a-warm-instance). The third is a secret that is
declared but not in the set you deployed with.

If Twilio's own call log shows nothing at all, the number is not pointed at the
Bin. Go back to [step 3](#3-point-the-number-at-your-agent).
...
the agent cuts itself off. The garbled turn then sits in its context for the rest
of the call.
...
before it opens the stream. Token authentication is structurally impossible here.
...
<Columns cols={2}>
  <Card title="Go live on Pipecat Cloud" icon="cloud-arrow-up" href="/deploy/pipecat-cloud">
