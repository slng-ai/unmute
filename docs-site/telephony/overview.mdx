...
| Pipecat `carrier-websocket` | Pipecat Cloud |
| Pipecat `daily-sip` | Pipecat Cloud, plus the public helper you host |
| Pipecat `sip` | nothing managed: you run the four containers yourself |
| LiveKit `sip` | LiveKit Cloud, or a LiveKit deployment of your own |
| LiveKit `connector` | LiveKit Cloud |

<Note>
  Pipecat `sip` is the one route with no managed platform anywhere in it, so it
  emits no deployment manifest. It also **receives calls only**: there is no
...
## Testing a route before you deploy it

Most of these routes run on your own machine with no carrier account at all, and
that is the loop to build in. [Call your agent
locally](/dev/local-telephony) is the one page for it: which plane each route
gets, what a local call proves, and what it cannot tell you.
...
<Columns cols={2}>
  ...
  <Card title="Inbound calls" icon="phone-incoming" href="/telephony/inbound-calls">
    A real call arriving on the route that works from a laptop.
  </Card>
  ...
  <Card title="Call your agent locally" icon="play" href="/dev/local-telephony">
    The whole local loop, with no carrier account.
  </Card>
</Columns>
