...
A connection has a full route like the one above or a receive-only route with
no credentials. A third carrierless `daily-sip` shape is outbound-only: it can
dial a transfer destination but cannot serve a phone channel.
[`connections/<name>.yaml`](/reference/connections-yaml) explains all three.
