...
<ParamField path="deployment_region" type="string, or a list of region names and region-to-swaps entries">
  One region or a duplicate-free list, on every target. LiveKit accepts
  `us-east`, `eu-central`, or `ap-south`. Pipecat forwards a non-empty platform
  region. Both use platform placement when omitted. SLNG requires at least one
  of its [13 world parts](/targets/slng#targets-yaml); no default is inferred.
  Several regions compile to one build each. A list entry may carry
