...
A target is one place your agent can run. Unmute generates two:
**Pipecat** and **LiveKit Agents**. Both are code targets, which means
`unmute compile` writes a Python project you own. The configuration also accepts
Vapi and Deepgram for validation only; no project is emitted for them.
...
| Field | What it does |
|---|---|
| `provider` | `pipecat` or `livekit`; `vapi` and `deepgram` validate only |
