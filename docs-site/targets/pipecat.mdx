...
A package that binds a [live model](/models/live) emits one service where the
transcriber, the model and the synthesizer would be built. It hears the caller's
audio directly, speaks in its own voice, and hands its tools to the `think` entry
it names. It is the one shape whose emitted `bot.py` builds no transcriber and no
synthesizer.
