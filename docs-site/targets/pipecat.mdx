...
<ParamField path="deployment_region" type="string, or a list of region names and region-to-swaps entries">
  Non-empty platform region names, as a string or a duplicate-free list.
  ...
  applies. Each region compiles to its own build, each with its own
  `pcc-deploy.toml`, agent name and secret-set instructions. A list entry may
