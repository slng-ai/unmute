...
<ParamField path="deployment_region" type="string, or a list of region names and region-to-swaps entries" required>
  Accepts `us-east`, `us-west`, `br`, `eu-west`, `eu-north`, `gb`, `za`, `il`, `jp`, `sg`, `id`, `in`, `au`. At least one region is
  required; omission and an unknown name are refused. The retired `any` value is
  refused. These are the same names speech and the router use. Each listed
  region is its own hosted agent, pushed under its own name. A list entry may
  carry [model swaps](/reference/targets-yaml#a-region-may-run-its-own-models).
</ParamField>
