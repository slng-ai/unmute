...
cp .env build/livekit/.env      # if the package-root .env already has the values
cd build/livekit
lk agent create --region eu-central --secrets-file .env
```

If no package-root `.env` exists, copy the generated `.env.example` to
`build/livekit/.env` and fill it in first. Choose a region from the field above.
The [numbered runbook](/deploy/livekit-cloud#the-first-deploy-is-create) also
explains what survives a recompile.
