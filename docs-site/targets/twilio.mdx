...
In the `unmute` console, switching the think provider loads that provider's
starter: `gpt-5.6-luna` with its two params for OpenAI, or
`gemini-3.1-flash-lite` with `thinking_level: MINIMAL` on the Developer API for
Google. Add `vertexai` and `location` yourself. Picking the provider that is
already bound keeps the binding as it is.
...
1. It reads the number
   ...
   SIP trunk or a voice fallback URL. It never clears these for you. It reads
   the copy of the number in the connection's `region`, and it refuses a
   number whose calls another region handles. It never changes the routing.
...
- A call behind a proxy other than a cloudflared quick tunnel.
- How Twilio plays tokens already sent when an interrupt arrives. One live
  interrupt was heard, on OpenAI.
- `end_call` on a real call since the app waits for Twilio to close the socket.
  The close order is tested offline only.
- A second call refused while `max_sessions` calls are running. The refusal
...
<CardGroup cols={2}>
  <Card title="Take a call with ConversationRelay" icon="phone-call" href="/telephony/twilio-conversation-relay">
    The example package, from compile to a real call, on OpenAI or Gemini.
  </Card>
