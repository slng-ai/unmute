...
5. To end the session, the app sends `end` and waits up to five seconds for
   ...
   error 64105. Twilio then asks `/connect-action`, and the app hangs up.
...
<ParamField path="region" type="us1 | ie1 | au1" default="us1">
  ...
  that region's token, because Twilio signs the region's requests with it. The
  number's routing region must match too. Refused on every other route.
</ParamField>
...
1. Copy `build/<target>/.env.example` to `.env` and fill it in.
...
- **Speech settings.** Deepgram transcribes and ElevenLabs speaks, both inside
  ConversationRelay. DTMF detection is off. With interruption on, the caller can
