...
4. It stops if the number already points at `https://<your origin>/voice`.
   With `--dry-run`, it stops before any write and prints the change.
5. It reads the number again and stops if its voice settings moved. Then it
   saves the old voice URL and method to a new private file under your user
   config directory, in `unmute/twilio-rollback/`.
...
7. It reads the number back, and writes `build/<target>/deploy-report.json`
   with the outcome: `routed`, `not_changed` or `unknown`. An `unknown` outcome
   exits 1 and prints the snapshot path.
...
To put the old route back, first check in the Console that the number still
points at the snapshot's `new_voice_url`. If it does not, somebody changed it
since, so stop and ask. If it does, set "A call comes in" back to the
snapshot's `voice_url` and `voice_method`.
