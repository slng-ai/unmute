...
Create the inbound trunk from that file the way you already do, and the
attribute appears on the caller. Nothing else to do.
...
Your token server also has to name the agent to dispatch. Coval does not send it.
The request body is only `room_name` and `participant_name`, even when the Coval
agent sets `livekit_agent_name`. A token with no dispatch leaves the room empty,
and the simulation fails with no transcript. So set the Coval agent's
`token_request_payload` to `{"agent_name": "<your agent name>"}`. Coval adds it
to the body, and your server dispatches that name.
...
Every Pipecat build also answers the runner's Twilio route, even a package with
no phone route. So Coval can call a bot on your laptop:

```sh
uv run bot.py -t twilio -x <public host>
```

Put a tunnel in front of port 7860. Then create a Coval websocket agent in TwiML
webhook mode, with `voice_url` set to `https://<public host>/`. Coval posts to
that webhook, reads the stream URL from the reply, and sends Twilio's `connected`
and `start` messages before any audio. The runner's reply carries no
`<Parameter>`, so on this path the trace does not find its simulation.
