...
create both after deploying, with two commands run from `build/livekit/`:

```sh
TRUNK=$(sed "s|\${SIP_FROM_NUMBER}|$NUMBER|g" sip-inbound-trunk.json |
  lk --project "<project-name>" sip inbound create - 2>&1 | grep -o 'ST_[A-Za-z0-9]*' | head -1)
sed "s|\${UNMUTE_SIP_TRUNK_ID}|$TRUNK|g" sip-dispatch-rule.json |
  lk --project "<project-name>" sip dispatch create -
```
