...
Several regions compile to one directory each, `build/<target>/<region>/`, and
each one holds a complete project with its own agent name. Run `lk agent create`
and later `lk agent deploy` from inside the directory for that region; the
platform writes a plain `livekit.toml` there on the first deploy.

Each region is its own agent name, so a caller reaches a region through a
dispatch rule that names it, not through LiveKit picking the nearest worker.
That is deliberate: a region can run its own models, and a caller routed to the
wrong region would get the wrong ones. See
[multi-region routing](https://docs.livekit.io/deploy/admin/regions/agent-deployment/)
for how the platform places workers.
