...
it from the same authored files. A Twilio target is an app you host yourself;
[the Twilio target](/targets/twilio) covers hosting it and pointing a number
at it. Validation checks your declarations;
...
Named initialization creates a LiveKit package. Run `unmute init` without a
name for the interactive target choices. Add `--from-manifest` to create the
package under a saved organization manifest instead; see
[initialization](/reference/cli/init#from-a-saved-manifest).
