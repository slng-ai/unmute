...
Cold transfer is a SIP REFER, so the trunk must permit transfers. Follow the
[Twilio Console setup](/telephony/twilio#set-up-the-livekit-sip-trunk-in-twilio-console)
to enable Call Transfer (SIP REFER), PSTN Transfer, and the Transferee caller ID.
