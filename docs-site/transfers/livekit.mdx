...
The route, every required environment variable, and the evidence for each
capability are all written to `build/livekit/compile-report.json`, under
`telephony`:

```json
{
  "telephony": {
    "route": { "provider": "livekit", "transport": "sip", "carrier": "twilio" },
    "coordination": "shared",
    "admission_owner": "livekit_dispatch",
    "evidence": [
      {
        "feature": "warm_transfer",
        "tag": "provisional",
