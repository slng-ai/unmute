...
The control's name goes in the agent's tool list, like any other control — and
that half is enforced, not merely conventional. See [the control has to be
attached](#the-control-has-to-be-attached) below.
...
A route is not the same as a phone channel, and the distinction is worth
holding on to: `examples/pipecat-human-transfer-daily` declares only
`web: realtime_audio` and transfers fine, because Daily's dial-out brings the
person into the room the browser session is already in. What it has is a
connection.

### The control has to be attached

Declaring a control under `controls:` is half the job. Until some agent lists
its name in `tools:`, no agent can reach it, and that is refused at build with
the file, the line, and the agents you could attach it to:
