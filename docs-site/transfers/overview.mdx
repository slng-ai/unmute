...
Unmute does not support warm transfer on any Pipecat target. Today, warm
transfer requires the LiveKit `sip` route.
...
declare only `web: realtime_audio` because Daily's dial-out brings the person
into the room. What it still needs is a connection.
