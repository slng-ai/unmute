...
### Every key an escalation takes

The name on the left is how an agent attaches it. Under the name are two parts:
when to run it, and which shape it is.

<ParamField path="when" type="string">
  What the model reads to decide to put the caller through. Nothing checks that
  it is there, but write one: without it the model has nothing to decide on.
</ParamField>

<ParamField path="cold" type="block">
  The cold shape: hand the caller over and drop out. Exactly one of `cold:` or
  `warm:` is written, never both and never neither.
</ParamField>

<ParamField path="warm" type="block">
  The warm shape: hold the caller, ring the person, brief them, then bridge the
  two. Only the LiveKit `sip` route compiles it.
</ParamField>

### Every key a shape block takes

`destination:` is required in both. `briefing:` is the one key `cold:` does not
have.

<ParamField path="destination" type="destinations entry name" required>
  Which desk to reach, named symbolically. The name is resolved in
  [`destinations:`](#destinations-are-symbolic) at the top level of `agent.yaml`.
</ParamField>

<ParamField path="briefing" type="string">
  What to tell the person while the caller waits. `warm:` only, so it cannot be
  written on a cold transfer. The drivers pass the call transcript alongside it.
</ParamField>

<ParamField path="ring_timeout" type="positive Go duration, e.g. 25s">
  How long to ring before the person counts as unavailable. Left out, Pipecat
  waits 25 seconds and LiveKit sets nothing, so its platform default applies.
</ParamField>

<ParamField path="on_unavailable" type="return_to_caller | hangup" default="return_to_caller">
  What happens when nobody picks up. Pipecat `cloud-websocket` requires
  `hangup` written out, because it cannot reconnect the original media stream.
</ParamField>
