...
| Pipecat `sip` | yes | **refused** | SIP REFER on the caller's existing leg, through the SIP service the route runs |
| Pipecat `daily-sip` + Twilio | yes, on a phone call | **refused** | Daily transfers the existing SIP phone leg |
...
- **no** means the platform has no primitive for it.
- **refused** means the platform has one and Unmute does not emit it, so the
  compiler rejects the shape and names where it does work.
