...
The route and the evidence for each capability are written to
`build/pipecat/compile-report.json`, under `telephony`:

```json
{
  "telephony": {
    "route": { "provider": "pipecat", "transport": "cloud-websocket", "carrier": "twilio" },
    "coordination": "shared",
    "admission_owner": "generated_runtime",
    "evidence": [
      {
        "feature": "cold_transfer",
        "tag": "provisional",
