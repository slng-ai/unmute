...
```text
livekit: binding reason.assistant_model provider=openai model=gpt-5.6-terra (forwarded as-is, not validated)
```
...
A warning is a real difference worth reading, not a failure: the command still
exits 0. Then talk to it in your browser:
