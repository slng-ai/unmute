...
<Tip>
  **If you want to run something before reading, start with
  `examples/salon-support`.** It is the smallest package that does something:
  web audio, local tools, no phone number, no third-party account, two keys. The
  package below is the one that teaches the file, and `salon-support` is the one
  that gets you talking in a minute.
</Tip>

## The agent, reduced

This is `examples/simple-prompt/agent.yaml` reduced, so the blocks this page
explains are the only ones on screen. Taken out: the five `tools:` and the
agent's tool list, which [Tools](/build/tools/overview) covers; the `tracing:`
block and its three `LANGFUSE_*` secrets, which are about traces rather than
about the agent; and the file's own opening comment. The `targets.yaml` below is
reduced the same way and its two targets are shown in the order this page
explains them. Everything else is the file as it is on disk — open the real one
alongside if you want the whole thing.
...
`params:` is passthrough for anything else the provider takes, and it is not
checked either. The one line here earns its place: `gpt-5.6-luna` is a reasoning
model, and OpenAI refuses a request that carries function tools unless it sets
`reasoning_effort`. This package has five tools, taken out of the listing above,
so the line is doing real work. [The reasoning model](/models/llm) has the rest.
