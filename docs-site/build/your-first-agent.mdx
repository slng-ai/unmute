...
This is `examples/simple-prompt/agent.yaml` with three things taken out, so the
blocks this page explains are the only ones on screen: the five `tools:` and the
...
about the agent. Everything below is otherwise the file as it is on disk. Open
the real one alongside if you want the whole thing.
