...
|---|---|---|
| `steps` | yes | one or more steps, in order: a bare task name, or an item with `task:` and `skip_when_confirmed:` |
| `when` | no, but write one | the situation the model reads to decide whether to run the group. Nothing checks that it is there, and a group without one gives the model nothing to decide on |
| `announce` | no | one fixed line, spoken exactly as written as the caller enters the group, to cover the wait before the first step speaks. It takes no `{{placeholders}}` |
