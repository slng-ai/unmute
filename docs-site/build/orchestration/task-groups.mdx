...
A group stops when a step ends unserved: the later steps do not run, and the
owner is handed the unserved status. Running the next step would answer a
question nobody asked.

A step's handoff also ends the group, and the caller is moved. When the model
calls a handoff in the same response as a tool that ends the step, the step's
own work is committed first and the caller is moved after: the booking is not
lost on the way out. If that work does not complete, the caller is not moved
