...
The model reads that clause every time it considers the step, next to the
`Conversation info:` block that shows whether an earlier step already
recorded the value. See [Order steps with the
