...
| Task | the task, until it returns | what `context:` gives it | 2 to enter, the same as one tool call | delegate again | a step with a definite, typed answer |
...
These are not exclusive. One agent can delegate a task in one phase and hand
off in another.
...
definite, typed answer: a task. The `verify_customer` delegate runs the
`customer_verification` task and assigns its result to `customer_phone`, and
`manage_booking` waits behind `requires: customer_phone` until it lands.
...
booking starts, not before browsing services, so `requires:` on one delegate
