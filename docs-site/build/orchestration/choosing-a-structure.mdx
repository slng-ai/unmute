...
| you need a value out of a step and want to keep it | a task that saves a declared variable with `assign:` |
...
same way, but it is cheap: no extra step, no extra prompt site, and nothing
...
pattern, including the tool-level check that catches a step run out of
order anyway.

Reach for the task group when the *order* is what must hold. Reach for a
`when:` clause when one *value* should usually be there before one step runs,
...
### Order the step, not the route to it

Say the dependency on the step itself, in its own `when:`:

```yaml
      - name: manage_booking
        when: The caller wants to make, change, or cancel a booking, once the caller is identified.
```

The model reads that clause every time it considers the step. Reference the
saved value in the owning prompt when the model needs to check it. See [Order steps with the
prompt](/best-practices/step-scoping#order-steps-with-the-prompt) for what
still catches a step run out of order anyway, and what the caller hears when
it does.

Do not add an agent whose only job is to hold callers up in front of a step.
That agent has to be spoken through, which costs the caller a turn and buys
nothing a clause on the step does not already give. And never hold up the
...
to `customer_phone`, and `manage_booking`'s own `when:` says it runs only
...
booking starts, not before browsing services, so a clause on one task's
`when:` is enough. A task group would buy nothing extra, and the package
declares none.
