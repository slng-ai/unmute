...
One tool is one entry, and one field is one item. Naming a tool twice, or
checking one field twice inside the same entry, is refused too: each of those
used to drop a set of conditions without saying anything, leaving a step that
never ended on its own.
