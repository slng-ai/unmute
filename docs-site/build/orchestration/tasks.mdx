...
`instructions:` points at a Markdown file. `tasks/` is a folder you create next
to `agent.yaml`, and the path is relative to the package.

`when:` tells the model when to run the task. A task with no `when:` is a
definition only: a task group can list it as a step, but an agent cannot run it
on its own.

`assign:` defines what the task saves and where each value goes. The
...
A task needs a code target, Pipecat or LiveKit Agents. On the SLNG target a
task cannot run at all, so every history value is refused there, `messages`
included.
