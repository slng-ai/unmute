...
These shapes need a code target, Pipecat or LiveKit Agents. The SLNG target
compiles one agent with one prompt, so it refuses tasks, task groups and
handoffs. `unmute validate` says so, and names the way out:
