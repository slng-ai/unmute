...
<ParamField path="architecture" type="cascade | realtime | live">
  Set `cascade`, or omit the key for the same result. Cascade is supported on all four targets, subject to each target's model and feature limits. It is the only architecture the Twilio target takes.
</ParamField>
