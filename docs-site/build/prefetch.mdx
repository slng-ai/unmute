...
doing it. `prefetch:` runs it once, before the greeting, and puts the answer in
a variable.
...
<Note>
  ...
  before it acts: [Check a value with the
...
### Choose a lookup with known inputs
...
This example uses the salon package's existing `look_up_customer` tool.
In your own package, declare a local or webhook tool with the corresponding
`phone` input and `name` and `status` output fields.

Add the ordered prefetch entries below and declare their receiving variables.
Each entry needs a name, one source, and assignments to existing variables.
...
Read a prefetched value only in prompts that need it:

```markdown tasks/booking.md
Local date: {{booking_date}}
Use this as today when the caller says tomorrow or names a weekday.
If no date is recorded, ask for the exact date before booking.
```

A prefetch assignment does not add a value to any prompt by itself.
...
<ParamField path="clock | source | tool" type="string" required>
  ...
  - `source: <fact>` reads a fact the call carries, such as `from_number`.
...
shaped text like `Phone`, `Date` or `EmailStr`, because the shape is checked
where the value enters the state. A `Literal` works when the tool's own field
declares the same set. A `list[...]`, a declared shape or `NameEmail` is
refused, and the message names the step to assign it from instead.

If you need an object or a list from a lookup, run it in a task and save the
result through task `assign:`. Prefetch currently supports scalar destinations
only, even if your backend already has structured records.
