...
| Tool kind | LiveKit | Pipecat | SLNG | Twilio |
|---|---|---|---|---|
| `webhook:` | yes | yes | yes | **no** |
...
| `client:` | no | no | no | no |
| `provider_hosted:` | no | no | no | no |

On Twilio, a `local:` tool needs flat `input` and `output` schemas, and the only
builtin is `end_call`. The target runs one agent with no tasks, and it refuses
`announce` and `interruption: cancel`. [The Twilio target](/targets/twilio#what-the-package-may-carry)
lists every refusal.
...
<AccordionGroup>
  ...
  <Accordion title="The rules">
    ...
    |---|---|
    | legal on `webhook:`, `local:`, `knowledge:`, `slng:` and `builtin:` | refused by tool name; an `mcp:` file is refused at load, with the line number |
    | on a `builtin:`, the `slng` target only | refused by name on a code target, which builds the prebuilt from its own SDK and has no seam in front of it |
