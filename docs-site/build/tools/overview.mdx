...
**`slng:` works everywhere because the definition travels.** `unmute pull` writes
the platform's own schema, and for a code tool its module, into your package. The
SLNG target then references the platform's copy by name, and the two code targets
build a real tool out of the committed mirror with no network. One limit is not in
this table: a hosted tool that declares Python dependencies is refused on LiveKit
and Pipecat, which build one dependency list for the whole project. See
...
|---|---|---|---|
| `webhook` | `url_env` | at least one of `url_env`, `base_url` | UPPER_SNAKE name holding the base URL; read by the code targets |
| `webhook` | `base_url` | at least one of `url_env`, `base_url`; required on SLNG | literal `https://` URL; read by SLNG, which stores the URL in the tool body instead of an environment |
...
| `mcp` | `server` | no | the server's name on the platform, when it is not this tool's own name. Read it with `voiceai mcp list` |
...
| `mcp` | `tools` | no | non-empty, unique server tool names; omitted means all tools. Required on slng, which attaches one reference per tool and has no "whole server" attachment |
...
A `webhook:` tool needs `url_env`, `base_url`, or both. Pipecat and LiveKit Agents
...
| `output` | no | illegal on `builtin:`, `mcp:` and `knowledge:`; author-side only elsewhere, not sent to the model, not included in the compile report, and not enforced at run time on the code targets |
...
**`output:` is refused on an slng target.** It used to be allowed there, and
allowed for a reason: the compiler turned it into a pydantic `Output` class
inside the code it uploaded, and SLNG read the tool's result shape off that
class. unmute uploads no code now, so the field reaches nothing and is refused
rather than dropped. A tool SLNG hosts carries its own `Output` model, written
where the tool was written. Full story in
[Python tools](/build/tools/python#what-the-slng-sandbox-expects) and
[Hosted tools](/build/tools/hosted).
...
to the model or puts it in the compile report, and the code targets never
enforce it at run time; SLNG enforces it, and only for a `local:` tool's own
handler. A task's required `result:` is enforced and describes what the whole
delegated task returns to its caller after any tool calls. It may select or
combine tool data, so design it for what the caller needs instead of copying a
tool output schema by default.
