...
| `mcp` | `url_env` | yes on LiveKit and Pipecat; not read by SLNG | UPPER_SNAKE name holding the server URL |
| `mcp` | `transport` | no | `sse` or `streamable_http`; omitted lets the SDK infer it from the URL |
| `mcp` | `auth` | no; never read by SLNG | bearer or API-key authentication |
| `mcp` | `tools` | no; required on slng | non-empty, unique server tool names; omitted means all tools. Required on slng, which attaches one reference per tool and has no "whole server" attachment |
