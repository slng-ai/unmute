...
**SLNG is the one exception, and only for `local:`.** There the compiler turns
`output:` into a pydantic `Output` class, and the generated handler returns
`Output(**result)`, so a Python handler's return value is checked against it.
A `webhook:` tool's `output:` stays documentation everywhere, SLNG included:
SLNG proxies that request rather than running code of its own, so it has
nothing to check the response against. Full story in
[Python tools](/build/tools/python#the-rules-the-function-follows).
