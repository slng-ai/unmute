...
Nothing here is checked offline: the description you write is checked only in
the sense that a compile records it as an override; whether the published
tool still means the same thing is a question only your organisation's copy
can answer, which is why it is one of the checks `unmute deploy` completes,
not one `unmute compile` does.
...
This snippet is separate from the phone-ready `slng-support` example. Declare
...
```sh
unmute validate examples/slng-support --target slng
...
the references they apply to, so a clean compile reads as "nothing wrong yet"
rather than "everything is fine." `unmute deploy` completes all three before
it changes the agent, and `--dry-run` reports them without changing anything.
See [Deploy to SLNG](/deploy/slng) for what that run checks and what it
prints.
