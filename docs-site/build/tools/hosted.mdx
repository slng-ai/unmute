...
<ParamField path="description" type="string">
  Overrides the platform's own. Omit it to inherit what the platform already
  says.
</ParamField>

<ParamField path="inject" type="list of pairs">
  Values merged into every call, never shown to the model.
</ParamField>

<ParamField path="announce" type="one sentence">
  One sentence the agent speaks as the tool starts.
</ParamField>

<ParamField path="effect" type="returns_data | ends_conversation">
  Whether the call ends the conversation.
</ParamField>

<ParamField path="interruption" type="provider_default | continue | cancel">
  Whether the caller can interrupt during the call.
</ParamField>
