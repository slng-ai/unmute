...
livekit, pipecat and slng, and nothing about the tool file changes between
them. The twilio target refuses hosted tools.
...
| a capability SLNG curates, like ending the call or reading the clock | `builtin:` |
...
A curated capability resolves either way: `slng: current_datetime` attaches it
by name, and so does `builtin: {id: current_datetime}`. Reach for `builtin:`
whenever the capability takes a setting of its own, because `inject:` under a
`slng:` block is an argument and a curated capability publishes no argument
schema to put one in. See [prebuilt tools](/build/tools/prebuilt).
