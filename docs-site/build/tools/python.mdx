...
That one is real too, from `examples/salon-support`. The fixtures above do not
use it, because they return instantly and a tool that speaks before doing nothing
slow is just noise. Full rules in
...
| it returns the value your description and prompt expect | the result goes back to the model; optional `output` is not enforced at run time |
