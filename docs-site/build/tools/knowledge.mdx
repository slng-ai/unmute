...
<ParamField path="documents" type="folder path" required>
  Folder path relative to the package root.
</ParamField>

<ParamField path="embed" type="embedding service" default="openai">
  One of the [embedding models](#embedding-models) below.
</ParamField>

<ParamField path="mode" type="meaning | keyword | hybrid" default="hybrid">
  Which kind of search a lookup runs. See
  [How the search works](#how-the-search-works).
</ParamField>

<ParamField path="chunk_size" type="integer" default="90">
  Passage size in **tokens**, 1 to 2048.
</ParamField>

<ParamField path="chunk_overlap" type="integer" default="20">
  Tokens two neighbouring passages share; 0 or more, and never larger than
  `chunk_size`.
</ParamField>

<ParamField path="top_k" type="integer" default="3">
  How many passages a lookup returns, 1 to 20.
</ParamField>

<ParamField path="min_score" type="number">
  Drop results scoring below this, 0 to 1. Absent by default. Read
  [Filtering by score](#filtering-by-score) before setting it.
</ParamField>

## The `knowledge:` block on a tool

<ParamField path="base" type="base name" required>
  The name of a knowledge base declared in `knowledge:`.
</ParamField>
