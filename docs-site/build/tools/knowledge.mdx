...
```text
livekit: tool "look_up_policy" names knowledge base "policy" which is not
  declared in knowledge: (declared: policies, pricing)
```

```text
livekit: knowledge base "policies": documents folder "knowledge/policies" holds no
  file of a supported type (looked for .txt, .md, .pdf)
```

```text
livekit: knowledge base "pricing" has unknown embed "cohere"
  (supported: bedrock, gemini, huggingface, openai)
```

```text
livekit: knowledge base "pricing" uses embed "openai", which needs
...
```text
livekit: knowledge base "pricing": chunk_overlap 240 is larger than chunk_size 220,
  which the splitter refuses: overlap is how much two neighbouring passages share,
  so it has to fit inside one
```

```text
livekit: knowledge base "pricing": chunk_size must be at least 1 token, got 0
livekit: knowledge base "pricing": top_k must be at least 1, got 0: a lookup that
  returns nothing has no reason to exist
```

A base declared and used by no tool is a **warning**, and the build succeeds. It
is worth saying, because it is read and indexed at every start and never queried.
...
```text
warning: knowledge "policies": skipping scanned-appendix.pdf, no text layer
knowledge "policies": 1 of 2 documents indexed
```
...
```text
knowledge "policies": no document yielded any text (scanned-policy.pdf).
...
```text
knowledge "policies": 1 document, embed openai, fixed until next compile
knowledge "pricing":  1 document, embed openai, fixed until next compile
```
