...
An id the registry does not hold is refused by name:

```
pipecat: tool "end_call" has unknown builtin "transfer_to_human"
```
...
```
pipecat: tool "end_call" builtin "end_call" fixes effect to ends_conversation, cannot be
  "returns_data"
```
...
```
pipecat: tool "end_call" builtin execution takes no input, output, handler, or url_env
```

**The description, unless you write one.** Leave `description` out and the registry
default is used. From a real compile of the bare file above:
...
<Columns cols={2}>
  <Card title="Variables" icon="braces" href="/build/variables">
    Personalize the call the agent just answered.
  </Card>
  <Card title="Tools overview" icon="wrench" href="/build/tools/overview">
    All six execution blocks, side by side.
  </Card>
</Columns>
