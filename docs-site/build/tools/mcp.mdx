...
carries no `url_env`, `transport` or `auth:`. The platform reads
the server by name from its own registration, so
...
| `url_env` | yes on LiveKit and Pipecat; not read by SLNG | the UPPER_SNAKE name of the environment variable holding the server address |
...
| `tools` | no; required on slng | non-empty, unique server tool names to offer; absent means all of them |
...
not expose is never offered, and the package still validates:
...
`pipecat-ai[cartesia,deepgram,mcp,openai,runner,silero,webrtc]==1.9.0`.
