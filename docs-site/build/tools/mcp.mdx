...
That is `examples/mcp-example`, which compiles on both targets.
...
Six fields that describe one tool to the model are illegal on an `mcp:` file.
...
The other four say the same thing about `output`, `inject`, `interruption`, and
`effect`. In one place, so you can see the whole rule at once:
...
An MCP source needs `livekit-agents` 1.6.0 or newer, because that is where the
emitted arguments are verified. A target that declares an older version fails
validation by name, rather than having its version quietly raised:

```text
livekit: livekit version "1.5.2" is too old for an MCP tool source, which needs livekit-agents >=1.6.0
```

**Pipecat needs nothing extra.** It emits one client per source, started with the
bot and closed on every shutdown path.

Either way the dependency the project declares picks up the extra for you:
`livekit-agents[mcp,openai]==1.6.10` and `pipecat-ai[mcp,openai,runner,silero,webrtc]==1.7.0`.

## What the compile does with it

Every name the block holds becomes part of the generated project. From a real
compile of `examples/mcp-example`:
...
And the tool source itself, on the agent that listed it:
...
The two platforms behave differently, and the generated README says so for your
build:

- **LiveKit** logs the toolset that failed and the agent still starts, without
  those tools.
