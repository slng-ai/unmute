...
<ParamField path="server" type="string">
  The server's name on the platform, when that is not this tool's own file name.
</ParamField>

<ParamField path="url_env" type="UPPER_SNAKE name">
  The name of the environment variable holding the server address. Required on
...
<ParamField path="auth" type="block">
  `bearer` with `token_env`, or `api_key` with `token_env` and an optional
  `header`. Never read by SLNG.
</ParamField>

<ParamField path="tools" type="list of names">
  Non-empty, unique server tool names to offer. Absent means all of them, and
  slng requires the list.
</ParamField>
...
<ParamField path="auth.type" type="bearer | api_key" required>
  Which scheme the request uses.
</ParamField>

<ParamField path="auth.token_env" type="UPPER_SNAKE name" required>
  The environment variable name holding the token.
</ParamField>

<ParamField path="auth.header" type="header name" default="X-API-Key">
...
**Fix:** put the UPPER_SNAKE variable name here and the address in your
environment.
