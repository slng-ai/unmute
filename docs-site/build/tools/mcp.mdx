...
`examples/mcp-example` pins `version: "1.6.4"` on that target, because the emitted
arguments are verified against the 1.6 series.
...
`livekit-agents[mcp,openai]>=1.6` and `pipecat-ai[mcp,openai,runner,silero,webrtc]==1.5.0`.
