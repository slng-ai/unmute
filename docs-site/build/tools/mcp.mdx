...
[`examples/slng-support`](https://github.com/slng-ai/unmute/tree/main/examples/slng-support):
its `tools/web_search.yaml` names the same server and the same two tools, and
carries no `auth:`. SLNG has nothing to do with it there: the platform reads
the server by name from its own registration, not from `url_env` or `auth`, so
those fields matter only for a code target. This package deploys to slng only,
agent name `support`, alongside three other tools, so there was nothing for
`auth:` to reach.
...
Where LiveKit and Pipecat pass the server through and let it decide what it
offers, SLNG writes one reference per tool up front, and compiling offline
means it cannot ask the server what those are:
...
`server`, with whatever credential it needs, so nothing in this block reaches
it at run time except the tool list. Those three fields matter for the code
targets only.
...
`livekit-agents[cartesia,deepgram,mcp,openai]==1.6.10` and
`pipecat-ai[cartesia,deepgram,mcp,openai,runner,silero,webrtc]==1.8.0`.
