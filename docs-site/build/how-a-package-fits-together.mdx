...
## Two small words that move data

Two keys touch your `variables:`. They look similar and do different jobs:
...
Luggage carries a value. A write-back fills it. In the example above,
`verify_customer` fills `customer_phone` with `assign:`, and `manage_booking`'s
own `when:` says it runs only once the caller is identified. That is how
"verify before booking" works, with no extra agent and nothing the caller can
hear: the order lives in the prompt, not in a gate.
