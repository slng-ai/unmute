...
| `tasks:` | run a smaller job with its own prompt, and use its typed result | yes, the task finishes and reports back |
...
Two keys touch your [`variables:`](/build/variables). They look similar and do
different jobs:

| Key | Job | Plain words | Legal on |
|---|---|---|---|
| [`context:`](/reference/agent-yaml#context) | luggage | "when the caller moves, these values travel along" | handoffs, tasks |
| [`assign:`](/build/orchestration/tasks) | a write-back | "when the task finishes, save its result here" | tasks |
