...
| `read_only` | `true` declares that this tool writes nothing, which [`prefetch:`](/reference/variables#prefetch-resolving-a-value-before-the-call-starts) requires before it may run the tool. **A promise, not a guarantee: the compiler cannot check it.** Legal on `webhook` and `local`; declaring it where no pre-fetch names the tool is a warning |
