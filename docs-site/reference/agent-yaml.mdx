...
| `variables` | `all` or a list of names | handoffs only |
...
| `input` | optional list of typed fields the receiving agent is handed, the same shape a task's `input` takes; its prompt ends with a block naming each |
| `context` | payload: `history`, and an optional `variables` selection |

A `context:` block needs `history`. `variables` is optional: left out means
`all`, and every declared value travels with the caller. A non-empty list of
declared variable names keeps those and resets every other value to its default
on the way across, and that list form compiles on livekit only.
