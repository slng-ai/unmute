...
`params` is passthrough. Nothing in it is checked, and a name the target's
settings object has no field for is not dropped: it rides that target's overflow
field and reaches the provider anyway. The `reasoning_effort` line above is the
