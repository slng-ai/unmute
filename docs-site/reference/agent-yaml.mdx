...
| `requires` | guard: declared variable names that must be set before the task runs. Also the only variables this task's own `instructions` may read beyond one that already has a value: naming a variable another task assigns without listing it here is a compile error |
...
| `assign` | write-back: a list of one-key `variable: result.<field>` pairs, copying fields from the task's result into declared variables |
