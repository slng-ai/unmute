...
| `models` | map | yes |  | sections `think`, `speak`, `listen`, `turn`, and on Pipecat `realtime` | model palette grouped by kind |
...
| `shapes` | list | no |  | one or more named field groups, `CapWords` names | field groups a variable's `type:` can refer to |
...
Five sections. The section an entry sits in decides its kind: `think` (LLM),
`speak` (TTS), `listen` (STT), `turn` (turn detection), and `realtime`, a
[live model](/models/realtime) that does the first three as one and compiles on
Pipecat only. The first four are maps keyed by entry name; `realtime` is a list
...
| `name`, `provider`, `model`, `voice`, `think`, `description` | `realtime`, and nothing else: every other field is refused on a live entry by name, with its line. `think` names a `models.think` entry with `provider: openai` that runs the model's tools and reasoning. See [Live model](/models/realtime) |
...
| `realtime` | a `models.realtime` entry name, in place of `think` and `speak`; the package then has this one agent and no tasks, see [Live model](/models/realtime) |
...
| `announce` | optional. One fixed line, spoken exactly as written as the task is entered, or spoken by the task itself when `opening` is `listen`. It takes no `{{placeholders}}` |
| `opening` | `generate` or `listen`; omitted means `generate`. `listen` speaks the `announce` line and waits for the caller, making no model request. See [End on a tool and open by listening](/build/orchestration/tasks#open-by-listening) |
| `finish` | a list of the tools that end this step. Each item is `tool:` and a `success:` list of one-key `field: value` items, where a value may be a list of alternatives. A tool returning a result that meets every pair ends the step and saves `assign:` from it. Every value has to be one the tool declares in its output `enum:` |
| `assign` | a list of one-key `variable: result.<field>` pairs. It derives the finish field's type and description from the destination and saves valid values together. The right side may be a dotted path into a declared shape |
...
| `think` | optional. A `models.think` entry name this task runs on instead of the default. LiveKit only: Pipecat and SLNG refuse it. With no `think:` the task runs on the entry agent's think profile, not on the profile of the agent that defines it |
| `context` | optional history choice; omission means `messages` |
...
| Field | Values | Notes |
|---|---|---|
| `history` | `full`, `messages`, `last_n`, `summary`, `reset` | optional; omission means `messages`. `summary` compiles on livekit only; slng refuses every value |
| `max_messages` | a positive integer | required with `last_n`; refused with other history choices |
| `summarizer` | a `models.think` entry name | required with `summary`; refused with other history choices |
| `include_tool_calls` | `true` or `false` | omitted behaves as `true` where the history mode keeps tool records; explicit `false` is LiveKit only |
...
| Field | Values |
|---|---|
| `steps` | the ordered run, at least one. Each step is a task name, or an item with `task:` and optionally `skip_when_confirmed:` naming a variable that step confirms. The group skips such a step when the variable is confirmed as the group starts |
| `when` | the situation the model reads to decide whether to run the group. Nothing checks that it is there, and a group without one gives the model nothing to decide on, so write one |
| `announce` | optional. One fixed line, spoken exactly as written as the group is entered. It takes no `{{placeholders}}` |
...
only, and it takes one setting from the package: `inject:` with a single
`from_number`, the sender as a literal number in international format starting
with a plus sign. The model supplies the recipient and the body itself, and
SLNG reads the Twilio credentials from your vault under `TWILIO_ACCOUNT_SID`
and `TWILIO_AUTH_TOKEN`, which `unmute deploy` checks.
