...
| `name` | string | yes |  | lowercase letters, digits, single hyphens; 3 to 64 characters | what the deployed agent is called |
...
| `shapes` | list | no |  | one or more named field groups, `CapWords` names | field groups a variable's, a result's, or an expected value's `type:` can refer to |
| `timezone` | string | before a `prefetch:` entry reads the clock |  | an IANA zone name, such as `Europe/Madrid` | the zone every clock reading in this package is taken in |
| `prefetch` | **list** | no |  | one entry per fact, each with a `name:` | facts resolved once per call, before the greeting, in the order written |
| `secrets` | list of strings | no |  | UPPER_SNAKE names | environment values the generated project reads |
| `destinations` | map of strings | when an escalation is used |  | lower snake case names to UPPER_SNAKE env names | phone destinations an escalation may use |
| `knowledge` | map | no |  | 3 to 64 characters of `[a-z0-9_]` | folders of documents a tool can search |
| `agents` | map | yes |  | one or more lower snake case names | agent prompts, models, nested tasks, and other callable names |
| `task_groups` | map | no |  | lower snake case names | ordered task sequences, named by the agents that run them |
| `handoffs` | map | no |  | lower snake case names | agent to agent, and never returns |
| `escalations` | map | no |  | lower snake case names | agent to a person |
...
| `fallback` | names in the same section; chains are flattened and cycles are refused. See [Reasoning model](/models/llm) and [Speech to text](/models/stt) |
...
| `source` | `call_start`, a system source, or left out |
| `default` | any value of the declared type |
| `confirm` | the step that must hear the caller agree before anything acts on this value |
| `description` | a note for readers of the file; not sent to the model |
...
A top-level list. Each item names one group of fields a variable's, a task's
`result:`, or an `expect:` field can then use as its `type:`. Full field
reference, the type grammar it shares with `result:` and `expect:`, and what
it takes to reach the model as structured data:
...
An IANA zone name. Required before a [`prefetch:`](#prefetch) entry reads the
clock: a container's own clock is UTC, so a business elsewhere needs this to
date a call correctly. Package level, because it is a fact about the business
rather than about one target.
...
date, a fact the call itself carries, or the result of one already-declared
read-only tool. Entries resolve top to bottom, and one that cannot resolve is
skipped rather than failing the call. Full field reference, the ordering rule,
...
|---|---|---|---|
| map key | yes |  | 3 to 64 characters of `[a-z0-9_]`. It is also the folder name inside the build and the search collection's name |
| `documents` | yes |  | folder path relative to the package root, holding `.txt`, `.md` or `.pdf` files |
| `embed` | no | `openai` | embedding service |
| `mode` | no | `hybrid` | `meaning` (vector), `keyword` (BM25), or `hybrid` |
| `chunk_size` | no | `90` | passage size in tokens, 1 to 2048 |
| `chunk_overlap` | no | `20` | tokens two neighbouring passages share; 0 or more, never larger than `chunk_size` |
| `top_k` | no | `3` | passages a lookup returns, 1 to 20 |
| `min_score` | no | none | drop results scoring below this, 0 to 1 |
...
needs care, and needs `mode: meaning` to do anything useful: these are similarity
scores, not probabilities, and in practice they land well below 1, so a value near
1 returns nothing, and on `hybrid` the unscored keyword results pass through any
cutoff. See [Knowledge bases](/build/tools/knowledge) for how to set it.
`top_k` times `chunk_size` is roughly what reaches
the model on every lookup, and the compiler warns above about 1500 tokens.

A tool reaches a base by name, with a
[`knowledge:` block](/build/tools/knowledge), and an agent reaches it by being
given that tool. There is no retrieval mode and no relevance threshold.
...
| `tools` | tool files this agent may call; definitions never go here |
| `tasks` | tasks this agent can run: each entry is either a full definition or a bare name for a task another agent already defines, see [`tasks`](#tasks) below |
| `task_groups` | entries under [`task_groups:`](#task_groups) this agent may run |
| `handoffs` | entries under [`handoffs:`](#handoffs-escalations) this agent may trigger |
| `escalations` | entries under [`escalations:`](#handoffs-escalations) this agent may trigger |
...
| Field | What it is |
|---|---|
| `name` | the task's name; unique across the whole package, checked before anything else |
| `instructions` | the task's own prompt file |
| `when` | the situation the model reads to decide whether to run the task. Absent makes the task a definition only, valid solely as a step of a task group; an agent naming such a task by bare name is refused |
| `announce` | optional exact spoken text, said as the task is entered; not spoken when a `requires:` guard holds the task back |
| `requires` | guard: declared variable names that must be set before the task runs, which waits until they hold a value. A task's or an agent's prompt may already read any declared variable, listed here or not |
| `expect` | optional list of typed fields the step expects when the agent runs it, each `- name: type` or a block with `name`, `type` and `description`. The agent fills them from the conversation it heard; a type ending in `\| None` marks one the agent may leave out. The step's prompt ends with a block naming each, and only that prompt may read them |
| `assign` | write-back: a list of one-key `variable: result.<field>` pairs; the right side may be a dotted path into a declared shape, picking one part of the result into a declared variable. See [variables](/reference/variables#assigning-a-task-result-to-a-variable) |
| `tools` | tool files this task may call; definitions never go here |
| `handoffs` | entries under [`handoffs:`](#handoffs-escalations) this task may trigger |
| `think` | optional override of the thinking model |
...
Every task needs a non-empty `result:`. Each field's type is the same
one-line [type grammar](/reference/variables#the-type-grammar) a variable's
`type:` takes: a scalar, `Literal["a", "b"]`, `list[...]`, or a shape declared
under [`shapes:`](#shapes). `string`, `number`, `integer`, `boolean`, and an
`{enum: [...]}` block still work, and mean what they always did.
...
| Field | Values | Notes |
|---|---|---|
| `history` | `full`, `messages`, `last_n`, `summary`, `reset` | required. `summary` compiles on livekit only; slng refuses every value, see [tasks](/build/orchestration/tasks) |
...
| `when` | the situation the model reads |
| `announce` | optional exact spoken text, said as the group is entered |
| `requires` | guard: declared variable names. The group does not run until every one holds a value |
...
| Field | Notes |
|---|---|
| `to` | required; names an entry under `agents:` |
| `when` | the situation the model reads |
| `announce` | optional exact spoken text |
| `requires` | guard: declared variable names |
| `expect` | optional list of typed fields the receiving agent expects, the same shape a task's `expect` takes; its prompt ends with a block naming each |
| `context` | payload: `history` |

A `context:` block needs `history`. Every declared variable travels with the
caller already, with nothing to write: see
[variables](/reference/variables#a-handoff-keeps-every-declared-value).
...
|---|---|
| `when` | the situation the model reads |
| `cold` | one of; hand the caller over and drop out |
| `warm` | one of; hold the caller, brief the person, then bridge |
...
| `input` | JSON Schema of the parameters the model fills; required except on `builtin`, `mcp` and `knowledge`, where it is illegal |
| `output` | optional author-side JSON Schema of the result; illegal on `builtin`, `mcp` and `knowledge`, and elsewhere not sent to the model, not emitted in the compile report, and not enforced at run time |
...
| `announce` | one fixed sentence spoken as the tool starts; no `{{variables}}`; legal on `webhook`, `local` and `knowledge`; absent means nothing is spoken |
| `read_only` | `true` declares that this tool writes nothing, which [`prefetch:`](/reference/variables#prefetch-resolves-a-value-before-the-call-starts) requires before it may run the tool. **A promise, not a guarantee: the compiler cannot check it.** Legal on `webhook` and `local`; declaring it where no pre-fetch names the tool is a warning |
...
| `webhook` | `url_env` or `base_url`, at least one: `url_env` to compile on LiveKit or Pipecat, `base_url` (a literal `https` host) to deploy on SLNG; optional `path` and `auth`; a non-empty path starts with `/` |
...
| `interruption.protect` | `greeting`, `tool_calls`, or both. Pipecat only. Names the stretches the caller cannot talk over while barge-in stays on elsewhere. A phone route protects the greeting by default, because a phone leg has no echo cancellation and the agent would otherwise hear its own opening line and cut itself off. Set `protect: []` for no protection. Refused alongside `enabled: false`, which already mutes the whole call. |
...
that lists it rather than in a top-level catalog. A task with no `when:` and
no task group naming it in `steps:` is a build error too: give it a `when:`
so an agent can decide to run it, or list it as a step of a task group that
is reached.
