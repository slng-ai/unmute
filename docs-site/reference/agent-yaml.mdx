...
scores, not probabilities, measured 0.21 to 0.62 on the example corpus, so a value
near 1 returns nothing, and on `hybrid` the unscored keyword results pass through
any cutoff. See [Knowledge bases](/build/tools/knowledge) for the measured cost of
each cutoff. `top_k` times `chunk_size` is roughly what reaches
