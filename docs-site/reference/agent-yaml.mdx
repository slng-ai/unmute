...
| `context` | payload: `history` plus a `variables` selection |

A `context:` block needs both `history` and `variables`. The `variables` value
is `all` or a non-empty list of declared variable names.
