...
| `delegates` | map | no |  | lower snake case names | steps that run a task or task group, then return |
...
| `delegates` | entries under [`delegates:`](#delegates-handoffs-escalations) this agent may trigger |
| `handoffs` | entries under [`handoffs:`](#delegates-handoffs-escalations) this agent may trigger |
| `escalations` | entries under [`escalations:`](#delegates-handoffs-escalations) this agent may trigger |
...
| `handoffs` | entries under [`handoffs:`](#delegates-handoffs-escalations) this task may trigger |
...
## `delegates`, `handoffs`, `escalations`

Everything the model can trigger besides a tool, in one block per kind. The
block an entry is written in is what it is, so there is no `kind:` field.
...
Runs a task or a task group, then control comes back.

| Field | Notes |
|---|---|
| `task` | one of; names an entry under `tasks:` |
| `group` | one of; names an entry under `task_groups:` |
| `when` | the situation the model reads |
| `announce` | optional exact spoken text, said as the step is entered |
| `requires` | guard: declared variable names |
| `assign` | write-back: `result.<field>` into a declared variable |

Exactly one of `task` or `group`. `assign` is legal only with `task:`, because
each mapping reads `result.<field>` from that one task.

`announce` covers the two model requests it takes to enter a step, which were
otherwise silence. It is exact spoken text, not a model instruction, and it is
**not spoken when the step is refused** for unmet prerequisites: a caller who
hears "let me pull up the diary" and is then asked for a phone number has been
told something untrue. Do not put one on a step whose first tool already
announces, or two lines queue in a row.
...
be reachable from `entry_agent`. An unused delegate, handoff, escalation,
destination, tool, task, task group, or non-entry agent is a build error. Attach
it to the reachable graph or remove it.
