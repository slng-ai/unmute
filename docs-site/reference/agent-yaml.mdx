...
| `announce` | optional exact spoken text, said as the task is entered |
...
`requires` is not a field here, on a task group, or on a handoff: a package
that writes it is refused as an unknown field. Order a step with the agent's
own numbered flow and the step's `when:` instead, see [Order steps with the
prompt](/best-practices/step-scoping#order-steps-with-the-prompt).
