...
<ParamField path="models" type="map" required>
  ...
  and on Pipecat `realtime`. Required; omission is refused.
</ParamField>
...
<ParamField path="realtime" type="string">
  Name of an entry in `models.realtime`, written in place of `think` and `speak`.
  The package then has this one agent and no tasks. Pipecat only; see
  [Live model](/models/realtime).
</ParamField>
