...
<CodeGroup>
```text Pipecat, Daily cold transfer
...
body itself: a tool's authentication block and a hosted tool's mirrored secret
names. Each of those names a vault entry by its bare name, with no braces and
no dollar. It reads the package's `secrets:` list nowhere, so a name declared
there that no tool authenticates with is never checked here. Declare it anyway
if the same package also targets livekit or pipecat: those two do read
`secrets:`.

A `{{$NAME}}` vault token is a different thing and does not belong in a prompt,
a greeting, or a tool field. SLNG substitutes one into an MCP server URL and
nowhere else; a prompt and a greeting are templates whose placeholder names
hold letters, digits and underscores, so the push refuses the dollar. Unmute
refuses such a token at validation, on every target. Use a package variable
written `{{"{{name}}"}}` for a value that varies per call, and let the tool
that needs a credential name its own vault entry.
