...
On a code target, your platform's secret store. Both LiveKit Cloud and
Pipecat Cloud take a secrets file on the first deploy and a one command
update after that. The generated `build/<target>/README.md` carries the
exact commands.
