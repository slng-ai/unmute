...
On a `pipecat` target the map validates with no error and nothing reads it, so
leave it out. An `slng` target refuses it by name: there is no generated
project whose packages could be pinned.
