...
<ParamField path="targets.<name>.deployment_region" type="string, or a list of region names and region-to-swaps entries">
  Where this target deploys. One region or a duplicate-free list, on every
  provider. LiveKit accepts `us-east`, `eu-central`, or `ap-south`. Pipecat
  forwards a non-empty platform region name. Both use platform placement when
  omitted. SLNG requires at least one of its
  [13 world parts](/targets/slng#targets-yaml); omission and `any` are refused.
  A list entry may carry model swaps, so that region runs its own models.
</ParamField>
...
`deployment_region` accepts one region or a duplicate-free list, on every
provider. One region compiles to `build/<target>/` and deploys as
`<name>-<target>`, exactly as before.

Several regions compile to `build/<target>/<region>/`, one complete build each,
deployed as `<name>-<target>-<region>`. `unmute compile` writes them all and
`unmute deploy` pushes them all. `unmute dev --region <region>` runs one
locally. A duplicate region and an empty entry are refused.

### A region may run its own models

Each region can read a different model entry under the same name. Declare the
regional model in `agent.yaml` beside the one it replaces, then name the pair in
the region:
...
`transcriber` and `voice` are the names the agents already use. In `us-west`
they read the entries as written. In `eu-north` they read `transcriber_fr` and
`voice_fr`. The agent, the prompt and every agent definition are the same in
both.

Both sides of a swap name an entry in this package's `models:`, and both must be
the same kind. Unmute refuses a name it cannot find, a swap across kinds, a swap
of a name no agent uses, and a `live` entry on either side.
...
See [Pipecat over Twilio](/telephony/pipecat-twilio#2-deploy-with-a-warm-instance)
