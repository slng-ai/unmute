...
<ParamField path="targets.<name>.provider" type="string" required>
  Target provider. Accepts `livekit`, `pipecat`, `slng`, `twilio`. Required; omission is refused.
</ParamField>
...
<ParamField path="targets.<name>.version" type="string">
  ...
  emits no framework project, and on Twilio, whose app runs on no framework.
</ParamField>
...
project for `livekit`, `pipecat` and `twilio`; a `slng` target compiles to a
deployment body instead, which [Deploy to SLNG](/deploy/slng) covers. A
`twilio` project is one app you host; see [the Twilio target](/targets/twilio).
...
provider "deepgram" was retired: the deepgram target never emitted a runnable project and
  ...
  slng/deepgram/nova:3-en. Supported providers: livekit, pipecat, slng, twilio
```
