...
Shapes start unset and lists start empty. Fill them through call-start data
or task assignments. An empty text value is treated as missing; a successful
...
| SLNG | Basic scalars: `str`, `int`, `float`, `bool`, and their aliases. Also the only target that takes `source: conversation`, a value the model records mid-call |
