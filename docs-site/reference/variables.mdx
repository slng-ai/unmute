...
| `conversation` | the model, during the call, once the caller has given and confirmed the value | `slng` only. It becomes one of the agent's runtime variables, filled through SLNG's own `set_runtime_variables` tool and returned on the call record under `memory_variables`. It takes no `default:` and needs a `description:`, which the model reads. It takes no `{{placeholder}}` in any prompt or the greeting. On `livekit` and `pipecat` a step's `assign:` records such a value instead |

A value the model records during the call reaches no prompt. It has no value
when the prompt is built, and SLNG rejects a prompt that names one:
...
That is how one is meant to work. Put the detail in `description:`, which the
model reads as it fills the value, and refer to the thing in prose instead of
with a placeholder.
