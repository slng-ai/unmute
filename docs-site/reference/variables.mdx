...
State is written only at a seam: before the call by `prefetch:`, and at a
task finish by `assign:`. Conversation reaches a task or handoff according to
...
`type` is a single line, written in Pydantic's own words. A task finish uses
the same type through the destination named by its `assign:`.
...
follows every rule a whole value already follows at that site: it has to be a
declared variable, a value still awaiting confirmation renders only in its
...
task validates the value and saves it atomically.
