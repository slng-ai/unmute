...
|---|---|---|---|---|---|
| `transport` | string | yes | — | `sip`, `connector`, `carrier-websocket`, `cloud-websocket`, `daily-sip`, as allowed by the target provider | mechanism that carries the call |
| `carrier` | string | when the route has a carrier leg | — | `twilio`, `telnyx`, or `plivo`, as allowed by the route | carrier account behind the route |
...
A carrierless Daily dial-out route. It is outbound dial-out only, so there is no
carrier leg to name and no carrier credentials to hold.

```yaml connections/daily.yaml
transport: daily-sip
```

That is the whole connection. It cannot receive calls and cannot serve a
`kind: telephony` channel; use it only in a package whose control dials a person.
The `DAILY_API_KEY` the runtime reads comes from the route's own environment, not
from your package, so it is not in `secrets:` and is still documented by the
generated project. This route also needs dial-out enabled on the Daily domain.
No compile-time check can see that permission, so the generated runbook says it.
...
The carrierless `daily-sip` dial-out form above accepts no `environment` keys and
is not a phone-channel route, so it is not a row in this table.
...
connections/twilio_sip.yaml:21: transport "sip" with carrier "twilio" is not a route for
  ...
  cloud-websocket with twilio; daily-sip with no carrier (outbound dial-out only; it
  cannot serve a phone channel), twilio.
```
