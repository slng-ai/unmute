...
<ParamField path="transport" type="string" required>
  ...
  `daily-sip`, `conversation-relay`, as allowed by the target provider. Required; omission is refused.
</ParamField>
...
<ParamField path="region" type="us1 | ie1 | au1" default="us1">
  Twilio Region that handles the calls, on a twilio `conversation-relay` route
  only. Refused on every other route. See [the Twilio target](/targets/twilio).
</ParamField>
...
On `conversation-relay`, `phone_number_sid` is read only by `unmute deploy`,
when it points the number at the app. The running app never reads it, so it is
not asked of the process.
