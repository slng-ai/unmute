...
A saved contract reaches a new package only when you ask for it:
`init <agent> --from-manifest` opens a picker, then guides the choices that
contract allows. Plain `init <agent>` writes the starter package and reads no
saved manifest. `manifest use` sets which contract the picker and the `unmute`
console offer first. See
