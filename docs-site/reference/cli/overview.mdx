...
It prints the release, the commit it was built from and that commit's date,
then a second line naming the framework versions this build supports. All of it
is stamped in at build time. `-v` is the short form.
