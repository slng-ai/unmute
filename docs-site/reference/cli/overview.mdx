...
| Command | What it does |
|---|---|
| [`manifest create`](/reference/cli/manifest) | edit and save a reusable manifest |
| [`manifest use`](/reference/cli/manifest) | choose the default for future agents |

`init` uses the saved default automatically. Add `--from-manifest` to pick a
different saved contract for one new agent. See [Manifest](/reference/manifest).
