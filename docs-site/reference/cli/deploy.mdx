...
Nothing here is ever created by unmute. SLNG owns tool creation entirely, and
...
<Warning>
A `builtin:` or `slng:` reference carries **the tool file's own name**, not the
builtin id or hosted name it selects. `tools/hang_up.yaml` declaring
...
```text
slng: <n> requirements satisfied
note: slng: tools/search_places_text.yaml supplies `query` from {{customer_name}}, which SLNG resolves when a call starts: the deploy checked that the parameter exists and accepts it, and the value itself is checked then
slng: credential from SLNG_API_KEY
slng: compiled examples/slng-support/build/slng (3 files)
...
included. `no mirror of it is committed` and `does not match the hash` are
