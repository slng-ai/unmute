---
title: "unmute pull"
description: "Optional, and only for a livekit or pipecat build: fetch a hosted tool's definition into your package so that compile can trust it offline. SLNG needs none of this."
---
...
One line per file, with what happened to it. `written` and `unchanged`
describe the mirror files. `pinned` is where the two reference forms differ:
for a **scalar** reference (`slng: check_order`), it means the generated
`tools/check_order.slng.meta.json` was written or updated, and the tool file
itself, `tools/check_order.yaml`, is never touched; for the **legacy** block
(`slng: {hash: ...}`), it means that tool file's own `hash:` line was:
