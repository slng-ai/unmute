...
Flags:
  ...
  -h, --help                 help for dev
      ...
      --region string        deployment region to run, for a target that names several (required without a TTY)
...
<ResponseField name="--target" type="string">
  ...
  this flag is not repeatable. A target that names several regions is several
  builds, so pair this with `--region`.
</ResponseField>

<ResponseField name="--region" type="string">
  Which region to run, for a target that names several in
  `deployment_region`. Each region is its own build, with its own models, so
  the region is part of choosing what to run. Not needed for a target that
  names one region or none. With several, a terminal gets a picker and a non
  terminal gets an error listing the choices.
</ResponseField>
