...
`params.world_part` uses the shared SLNG region list:

| Group | Regions |
|---|---|
| Americas | `us-east`, `us-west`, `br` |
| Europe | `eu-west`, `eu-north`, `gb`, `za`, `il` |
| Asia | `jp`, `sg`, `id`, `in`, `au` |

These names also apply to speech and SLNG deployment. Choose a concrete region;
`any` and the old router names are no longer accepted.
...
| Key | What happens |
|---|---|
| `world_part` | consumed, and becomes the router's regional base URL |
