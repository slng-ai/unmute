...
header, and the bound is 128 characters, counted on the finished scope rather
than on the `agent_id` alone. The message names which agent or task produced
the long value, so you know which name to shorten. It refuses rather than
...
reaches OpenAI directly, which makes it a starting point rather than a finished
demonstration. It has several prompt sites, so pointing its think binding at
the router gives you one scope per site to look at.
