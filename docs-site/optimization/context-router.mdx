...
- [Regions](#regions) - the four router endpoints
...
<ParamField path="params" type="block">
  Two names are consumed here and everything else rides the request body. See
...
<ParamField path="params.world_part_override" type="eu | us | india | indonesia">
  Consumed, and becomes the router's regional base URL.
</ParamField>
...
`customer_name` variable. The router receives the prompt with the placeholder
...
It refuses a value the router does not serve and names the four. A speech
gateway value is the usual cause, because the two sets look alike and are not.

**Fix:** write `eu`, `us`, `india` or `indonesia`. A speech model's
`params.world_part` keeps its own
[13 values](/optimization/regional-infrastructure#choose-a-speech-gateway).
