...
a value learned partway through is in the prompt from the next turn on. Any
agent or task prompt may already name a variable with no value yet, whether
its think binding routes through here or straight to a provider: an unset
variable renders as nothing rather than leaving a compile-time hole. What
differs on a router-bound prompt is only the timing of the read. A
direct-provider prompt renders locally, on entry or on the way into a step,
so it only catches up once something re-renders it. A router-bound prompt is
never rendered here at all: it keeps its placeholder, and the router fills it
fresh on every single request, so a value learned mid-call reaches it without
