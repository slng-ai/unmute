...
<Warning>
  These four are the router's own regions. SLNG **speech gateways** use a
  different set under the same `world_part_override` key: for example, `in`
  selects the Indian speech gateway, while this router requires `india`.
  See [speech gateway values](/optimization/regional-infrastructure#choose-a-speech-gateway).
  The compiler refuses a value the router does not serve and names the four.
</Warning>
...
reaches OpenAI directly, which makes it a useful starting point rather than a
finished demonstration: it is a package with several prompt sites, so pointing
its think binding at the router gives you one scope per site to look at.
