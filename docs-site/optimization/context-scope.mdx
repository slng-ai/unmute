...
In the owning agent's instructions, make the order clear:
...
There is no automatic request passed by the owning agent.
...
[Variables](/build/variables#group-related-fields-into-a-shape), and add:
...
caller request. Let the owning agent confirm the change.
```

The owning agent and any later agent that needs the booking should read it:
