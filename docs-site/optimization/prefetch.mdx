...
with no change to the step, the delegate or the routing.
...
- **`result:` and `assign:`** do the same for a step's own return. A task can
  come back with several typed fields, and the delegate that ran it keeps only
  the ones it assigns to a variable. The rest did their job inside the step and
...
at all, so no pre-fetch can help. That one gets an `announce:` on the delegate
instead.
...
delegate into the next. The caller hears two cover lines for one thing they
asked for.
