...
A tool that silently reads an empty value refuses to run and names the step
that would supply it. Once a `prefetch:` entry fills the value directly, that
tool call succeeds on the first try, so the model never has to detour through
a step that existed only to collect it. Add a `prefetch:` entry and the
detour disappears, with no change to the step or the routing.
...
A `prefetch:` entry pointed at a tool that is not `read_only: true` is
refused the same way. The default flips from "every value sits in front of
every prompt, because that was easier" to "a value goes where the file says
it goes, and the build checks".
