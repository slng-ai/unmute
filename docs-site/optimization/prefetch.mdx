...
description: "How to decide which lookups belong before the greeting, and the traps we hit moving them there."
...
One call proves nothing. Two calls on the same build have come out more than a
whole round trip apart, just from run to run.

Make at least three calls per setup, and compare **the part of the call you
changed** rather than the total. A removed round trip is one of the few changes
clear enough to see in a single comparison: count the `llm_request` spans inside
the step you changed, and the count should have dropped by a whole number.

The agent also logs one line per entry, which is the quickest way to see whether a
pre-fetch got its value, was skipped, or ran out of time:
