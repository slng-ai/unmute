...
If a lookup is usually slow, leave it as a tool called during the conversation.
...
Use an announcement only for waits that need one. And never let two things speak for one
