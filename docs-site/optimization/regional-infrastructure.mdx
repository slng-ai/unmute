...
| Model gateway | An API that routes requests to a model service. | The gateway's own location setting; SLNG speech uses `params.world_part_override`. |
...
**SLNG provides 13 regional speech gateways through one model setting.** On
LiveKit and Pipecat, set `params.world_part_override` on each SLNG `listen` or
`speak` model to choose its gateway. This gives both speech roles the same
configuration shape across the two frameworks.
...
services. Here, it emits `slng_base_url="eu-north.api.slng.ai"` on LiveKit and
...
`world_part_override` rather than passing it to the plugin.
...
`eu`, `us`, `india`, and `indonesia`. It uses the same `world_part_override`
key but a different set of values: speech uses `in`, for example, while the
router uses `india`. The LLM behind the router keeps its own provider settings.
