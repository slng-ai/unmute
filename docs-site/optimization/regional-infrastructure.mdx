...
[Context Router](/optimization/context-router) uses the same 13 regions as SLNG
speech and deployment. Set `params.world_part` to choose the router region.
The LLM behind the router keeps its own provider settings.
