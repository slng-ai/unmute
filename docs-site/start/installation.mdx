  ...
  **Build from a clone**
  ...
  <Steps>
    ...
    <Step title="Check it">
      ...
      It prints the release, the commit it was built from and that commit's
      date, then a second line naming the framework versions this build
      supports. A plain `go build` with no link-time stamping prints
      `unmute version dev`.
    </Step>
  </Steps>
