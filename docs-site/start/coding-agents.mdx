...
comes back. Setup is quick, and talking to the agent it builds takes a bit
longer.
...
A correct answer names all six: an HTTP `webhook:`, a `local:` Python handler
in the package, a remote `mcp:` server, `builtin:`, which selects a tool the
runtime already has by id, `knowledge:`, a search over a folder of your own
documents, and `slng:`, a reference to a tool the SLNG platform already hosts.
A very good answer also mentions the two blocks that exist in the schema but no
target emits.
