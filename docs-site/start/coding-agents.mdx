...
```text
In Unmute, what are the five ways a tool can run?
```

A correct answer names all five: an HTTP `webhook:`, a `local:` Python handler
in the package, a remote `mcp:` server, `builtin:`, which selects a tool the
runtime already has by id, and `knowledge:`, a search over a folder of your own
documents. A very good answer also mentions the two blocks that exist in the
...
|---|---|---|
| One page as Markdown | add `.md` to any page URL, for example [`/start/coding-agents.md`](https://unmute.mintlify.app/start/coding-agents.md) | pointing an assistant at the one page that answers the question in front of it |
| The index | [`/llms.txt`](https://unmute.mintlify.app/llms.txt) | letting an assistant see every page and its description, then fetch the ones it needs |
| The whole site | [`/llms-full.txt`](https://unmute.mintlify.app/llms-full.txt) | one file with every page in it, for a tool that wants the lot in a single fetch |
...
`examples/simple-prompt` in the repository, which the test suite validates and
