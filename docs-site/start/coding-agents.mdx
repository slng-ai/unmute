...
```text expandable
  .agents/skills/unmute/
    ...
    references/manifests.md          written
  ...
  .claude/skills/unmute/
    SKILL.md                         written
  .agents/skills/unmute-manifest/
    SKILL.md                         written
  .claude/skills/unmute-manifest/
    SKILL.md                         written
  .agents/skills/unmute-deploy/
    SKILL.md                         written
    references/slng-push.md          written
  .claude/skills/unmute-deploy/
    SKILL.md                         written

Installed the Unmute skills for claude, codex, copilot, cursor.
...
but the same instructions. Commit `.agents/` and `.claude/`. Then anyone who clones the
repository has an assistant that knows Unmute, with nothing to install.

Three skills are installed, because they are three jobs. `unmute` builds and
maintains agent packages. `unmute-manifest` interviews you about your company's
approved choices and saves the contract. `unmute-deploy` pushes a finished
package to SLNG and reads back what landed. Your assistant reads the
...
Each file under `.claude/skills/` is a pointer at the matching bundle, so the
...
<Tip>
  ...
  offline. It writes only the directories above, so if your assistant
