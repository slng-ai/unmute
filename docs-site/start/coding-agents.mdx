---
title: "Coding agents"
description: "Install the Unmute skill, then build a voice agent by asking your coding assistant for one."
---

Unmute ships a skill that teaches a coding assistant how to write an Unmute
package: the schema, the model catalog, the tool kinds, how to shape a prompt
for speech, and what each target refuses. Install it once, then describe the
agent you want in a sentence and check what comes back. Setup takes about a
minute. Getting to a voice you can talk to takes about fifteen.
...
at Unmute, so prove it before you build anything. Ask:
...
5. **Ask what it decided.** Target, models, and the context across a handoff.
   A silent default is the bug you find a week later.
6. **Listen to the agent before you ship it.** A green check is not a good
   call.

## Where next

- [Orchestration](/build/orchestration/overview) for when one agent and a
  growing prompt stop being enough.
- [Tools](/build/tools/overview) for the four kinds in full.
- [Telephony](/telephony/overview) for putting it on a real phone number.
- [Deployment](/deploy/going-live) for getting it off your laptop.
