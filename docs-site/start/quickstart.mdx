...
import { unmuteVersion } from '/snippets/unmute-version.mdx';
...
This page was written for Unmute **{unmuteVersion}**. Run `unmute --version` to
see what you have.
...
<AccordionGroup>
  <Accordion title="docker compose is required to run unmute dev">
    ...
    command again. This scaffold targets LiveKit, whose local server stack runs
    in Docker Compose. Pipecat browser targets use `uv` instead.
  </Accordion>
