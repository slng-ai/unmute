...
Cartesia, Soniox and Speechmatics all do. While `turn:` names the local pair
above, those services only *propose* an ending; the pair still decides. So
picking one of them does not change what `endpointing_delay` and `pace` mean,
and does not give you a second thing racing your settings.

### Let the transcriber decide

Two transcribers can take the whole decision, and one of them can also predict
...
Any other listening vendor is refused with this list, because those two are
the only services the framework ships that both detect and predict a turn.
A Deepgram model that is not a Flux model, or `ink-whisper` on Cartesia, is
refused too: the turn-detecting class serves its own model family.

**What `pace` means here.** The ceiling becomes the transcriber's own end-of-turn
timeout, in milliseconds: `snappy` is 1200, `balanced` 1600, `patient` 3000.
There is no floor, because no local silence window ends a turn, so
...
the local detector predicts nothing.
