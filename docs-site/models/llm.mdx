...
not. Write them here rather than in a target override. A per-target `models:`
entry replaces the base entry instead of merging into it, so an override would
have to repeat `provider`, `model` and `reasoning_effort` to keep them, and two
copies of one binding is a binding that gets edited on one side. Pipecat drops
both and builds `OpenAILLMService` either way, and `unmute validate` prints a
warning per param naming the target, so you are told what did not apply. Both
salon examples author all three here.
