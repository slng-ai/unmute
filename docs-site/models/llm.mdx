...
- [Native Gemini in the EU](#native-gemini-in-the-eu) - Google Vertex with your API key
...
Set `GOOGLE_API_KEY` in the package's `.env`. This Vertex API-key path supports
`location: eu` and pins every model request to
`https://aiplatform.eu.rep.googleapis.com`. It has no global fallback.
...
<ParamField path="params.location" type="string">
  Set to `eu` with `vertexai: true`. A location without Vertex is refused.
</ParamField>
