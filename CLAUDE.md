...
| `internal/voice-agents-tests/salon-concierge-v2` keeps its three declared shapes, its typed values, the nested shape the `$defs` question needs and the two steps that append rather than replace | `internal/generate/examples_test.go` (`TestSalonConciergeV2ScopesEveryStep`) |
