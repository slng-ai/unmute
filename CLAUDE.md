...
### Typed inputs
A task's or a handoff's `input:` is a **list** of fields in the same two forms
a shape's `fields:` take, read by the same decoder. An input is not declared
...
both targets; the compiler, not the runtime, keeps an input out of every other
...
| an `input:` item on a task or a handoff decodes in both forms a shape's field takes, through the one decoder that exists, and an item holding two keys or none is refused with its line; the derived schema publishes `input` on both and requires it on neither | `internal/spec/input_test.go` |
| an input named like a declared variable, a shape, a grammar word or `unserved_request` is refused naming both, a name declared twice on one site or with two types across the package is refused naming both sites, and `input:` on a grouped task is refused naming the group: at run time the name is a field on the shared call state, so a collision would overwrite the other value for the visit | `internal/ir/input_test.go` (`TestBuildRefusesEveryCollidingInputName`, `TestBuildRefusesInputOnAGroupedTask`) |
| **a prompt naming an input it was not handed is refused, naming the one prompt that may read it**, because at run time the value sits on the shared state and nothing else keeps a step's request out of its parent's prompt; a tool's `inject:` may read an input only when it is required and every site the tool is attached to is handed it | `internal/ir/input_test.go` (`TestCheckTemplatesRefusesAPromptNamingAnInputItWasNotHanded`, `TestToolInjectMayReadARequiredInputOfEverySiteItIsAttachedTo`) |
...
| a package declaring no `input:` emits **byte-identical** output on both targets: no type table, no helper, no block, no parameter, no save or restore, no explicit schema, no runbook section. `_FINISH_TYPES` keeps its name for this reason, and `_INPUT_TYPES` sits beside it | `internal/generate/typed_inputs_test.go` (`TestTypedInputsEmitNothingForAPackageThatDeclaresNone`), over every `examples/` package and `salon-concierge-v2`, plus the golden files |
| the request block and the input machinery are byte-identical on both targets, because both are rendered once above the drivers | `internal/generate/typed_inputs_test.go` (`TestRequestBlockIsByteIdenticalOnBothTargets`, `TestTypedInputMachineryIsByteIdenticalOnBothTargets`) |
| a step's inputs are validated before the step exists and before anything is spoken, refused naming the field with the previous contents standing, written to the shared state for the visit and **put back, never cleared**, after it, so an owner whose own brief shares a name keeps it; the restore does not run on the in-task handoff path, where the receiver's brief was just written | `internal/generate/typed_inputs_test.go` (`TestAnInputOutsideItsTypeIsRefusedBeforeTheStepIsConstructed`, `TestAStepsInputsAreSavedAndRestoredAroundTheVisit`) |
| a handoff validates its brief, resets every input the receiver's prompt reads, writes its own values and only then builds or activates the receiver, on the agent seam and the in-task seam alike; a receiving agent's brief survives its own steps; a Pipecat worker with a brief and no steps re-renders its prompt on activation | `internal/generate/typed_inputs_test.go` (`TestAHandoffWritesTheBriefBeforeTheReceiverExists`, `TestAReceivingAgentsBriefSurvivesItsOwnStep`, `TestPipecatWorkerWithABriefAndNoStepsRerendersOnActivation`) |
| a required input is required in both emitted schemas and an optional one is not, required parameters precede optional ones, and a Pipecat delegate or handoff with inputs advertises an explicit `FunctionSchema` spliced from the generated types while one without keeps the signature path, because that framework reads a schema off a Python signature and a signature cannot express a `Literal` or a declared class | `internal/generate/typed_inputs_test.go` (`TestInputsReachBothDelegateSchemas`) |
| every prompt naming an input reads whole with that input rendered as `not given.`; input names join the router's template-variable names on both targets; the emitted runbook names the request block per receiver | `internal/generate/typed_inputs_test.go` (`TestInputPromptsReadWholeWithEveryValueEmpty`, `TestSlngRouterCarriesInputNamesOnBothTargets`, `TestRunbookNamesTheRequestBlockPerReceiver`) |
| `input:` is refused on the slng target naming the target and the alternative, and the same package validates clean on both code targets | `internal/ir/input_test.go` (`TestSlngRefusesAnInput`) |
...
| `input:` on a task and on a handoff round-trips through `unmute maintain`, both authored forms | `internal/tui/typed_inputs_roundtrip_test.go` (`TestMaintainKeepsATasksInputs`, `TestMaintainKeepsAHandoffsInputs`) |
| `internal/voice-agents-tests/salon-concierge-v3` declares inputs on its four seams, runs every step and handoff on `reset`, declares no `caller_reason` and no `reason:` result, writes no `variables:`, and validates and generates on both targets, while `salon-concierge-v2` stays the frozen control | `internal/generate/examples_test.go` (`TestSalonConciergeV3HandsEverySeamItsRequest`, `TestSalonConciergeV2ScopesEveryStep`) |
...
session state and is the control, and `salon-concierge-v3` is the typed-inputs
