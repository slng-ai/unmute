...
### Typed session state
A variable's and a task result field's `type:` is a single-line Python type
expression, and `shapes:` is a top-level **list** of named field groups it can
refer to. The grammar and the column of anything malformed inside an expression
live in `internal/spec/typeexpr.go`; the vocabulary, meaning which names are
types and what to write instead of each one that is not, lives in
`internal/ir/shapes.go`. That split is why one refusal can print both the file's
line and the column inside the expression. The emitted Python for the whole
feature is written once in `internal/generate/shapes.go` and inserted into both
targets verbatim, following `prefetch.go`, so the two cannot
drift; the block that reaches every prompt is composed once in
`internal/ir/state_block.go`, above both drivers, for the same reason.

### Expected values
A task's or a handoff's `expect:` is a **list** of fields in the same two forms
a shape's `fields:` take, read by the same decoder. An expected value is not declared
state: it is a field on the one call-state object for its visit, written by the
delegate or handoff tool before the receiver exists and put back after, because
every renderer on both targets and the router's template variables read that
object and nothing else. That placement is what makes the block identical on
both targets; the compiler, not the runtime, keeps an expected value out of every other
prompt (`checkTemplateSite` in `internal/ir/variables.go`). Resolution and the
block live in `internal/ir/input_block.go`; the emitted Python is in
`internal/generate/shapes.go` beside the finish table, which keeps its name so a
package handing nothing in emits the bytes it always emitted.
...
| a committed mirror is checked against the pin its tool file records, offline, at every compile, and the algorithm is pinned so a change of hash cannot silently invalidate every pin already committed | `internal/ir/hosted_test.go` (`TestMirrorPinIsCheckedOffline`, `TestHostedDigestAlgorithmIsPinned`), and `internal/cli/compile_test.go` (`TestCompileNeedsNoCredential`) proves the whole compile needs no credential and no `voiceai` on PATH |
...
| a package declaring no shape and no structured type emits **byte-identical** output on both targets: no block, no class, no import, no list default | `internal/generate/typed_state_test.go` (`TestTypedStateEmitsNothingForAPackageThatDeclaresNone`), `internal/ir/state_block_test.go` (`TestStateBlockIsEmptyForAPackageWithNoShapes`, `TestBuildLeavesAPrimitiveVariableAlone`) plus the golden files |
| **the composed state block omits a value awaiting the caller's agreement at every site but its confirming step's**, because `checkTemplates` walks the raw authored markdown and refusal 16 is blind to a composed block | `internal/ir/state_block_test.go` (`TestStateBlockOmitsUnconfirmedAtEveryOtherSite`) |
| no declared secret reaches a composed block, and no placeholder in one carries a dot, because the emitted substitution regex tokenises flat identifiers only | `internal/ir/state_block_test.go` (`TestStateBlockNamesNoDeclaredSecret`, `TestStateBlockNamesNoDottedPath`) |
| the block reaches every agent prompt and every task prompt, in declaration order, with no prompt file edited | `internal/ir/state_block_test.go` (`TestStateBlockReachesEveryPromptInDeclarationOrder`) |
...
| the composed block and the declared-state Python are byte-identical on both targets, because both are rendered once above the drivers | `internal/generate/typed_state_test.go` (`TestComposedStateBlockIsIdenticalOnBothTargets`, `TestTypedStateBlockIsByteIdenticalOnBothTargets`) |
...
| a shaped text value accepts the **empty string** and still refuses a wrong one, because empty is how a declared value says nothing yet: it is a variable's `default:`, it is what the state block renders as words, and it is what a tool hands back for a field it could not fill. Refusing it deadlocked a live call on both targets, differently: LiveKit logged a generic `error parsing arguments for finish`, and Pipecat's refusal reached the model, which asked the caller out loud for an identifier no tool returns | `internal/generate/typed_state_test.go` (`TestShapedTextAcceptsNoValueYet`) |
| a shaped text type tells the model its format, through the one keyword that can reach it: a `description` on the emitted alias, since a `format` or a `pattern` is stripped or refused. One phrase serves both it and the refusal, so the two cannot name different formats, and a field carrying its own description gets the phrase appended because Pydantic drops the alias's. Left off, the format was learned from a refusal mid-call: a live call spent a model round trip sending `"11:30 AM"` for a `Time` while its own prompt requires that spelling out loud | `internal/generate/typed_state_test.go` (`TestShapedTextTellsTheModelItsFormat`) |
| every append onto a declared list goes through one emitted helper, which drops an absent entry and a structured entry already on the list, because a step re-entered mid-call reads a value out of its own state block and hands it straight back: one live call entered the booking step four times, finished three of them at once with the appointment the first had recorded, and the caller's recap listed one booking four times. A plain value still appends every time, since two bookings really do give two reasons of `create_booking` | `internal/generate/typed_state_test.go` (`TestAnAbsentEntryAppendsNothing`, `TestTwoAppendedEntriesAreBothRecorded`) |
...
| an agent's prompt is re-rendered after one of its own steps writes declared state, so the conversation state block holds what the step recorded. LiveKit renders in `on_enter` and an agent is entered once per call, so the block froze at "none recorded yet" while the steps, entered per visit, read the real values; Pipecat never had it because restoring the owner's system prompt after a step re-renders it. Masked until a package moved off the SLNG router, which substitutes per request | `internal/generate/state_refresh_test.go` (`TestLiveKitRefreshesTheOwnerPromptAfterAStepWritesState`, `TestBothTargetsRefreshDeclaredStateWithoutWaitingForAnEntry`, `TestNoRefreshEmittedForAPackageWhoseStepsAssignNothing`), verified by deleting the call and watching both fail |
...
| an `expect:` item on a task or a handoff decodes in both forms a shape's field takes, through the one decoder that exists, and an item holding two keys or none is refused with its line; the derived schema publishes `expect` on both and requires it on neither | `internal/spec/input_test.go` |
| an expected value named like a declared variable, a shape, a grammar word or `unserved_request` is refused naming both, a name declared twice on one site or with two types across the package is refused naming both sites, and `expect:` on a grouped task is refused naming the group: at run time the name is a field on the shared call state, so a collision would overwrite the other value for the visit | `internal/ir/input_test.go` (`TestBuildRefusesEveryCollidingInputName`, `TestBuildRefusesInputOnAGroupedTask`) |
| **a prompt naming an expected value it was not handed is refused, naming the one prompt that may read it**, because at run time the value sits on the shared state and nothing else keeps a step's request out of its parent's prompt; a tool's `inject:` may read an expected value only when it is required and every site the tool is attached to is handed it | `internal/ir/input_test.go` (`TestCheckTemplatesRefusesAPromptNamingAnInputItWasNotHanded`, `TestToolInjectMayReadARequiredInputOfEverySiteItIsAttachedTo`) |
| the request block ends the receiving prompt after the state block, in authored order, reaches no other prompt, names no dotted path, and a receiving agent's block is the union of the handoffs that target it | `internal/ir/input_test.go`, `internal/generate/typed_inputs_test.go` (`TestRequestBlockReachesTheStepPromptOnBothTargets`) |
| a package declaring no `expect:` emits **byte-identical** output on both targets: no type table, no helper, no block, no parameter, no save or restore, no explicit schema, no runbook section. `_FINISH_TYPES` keeps its name for this reason, and `_INPUT_TYPES` sits beside it | `internal/generate/typed_inputs_test.go` (`TestTypedInputsEmitNothingForAPackageThatDeclaresNone`), over every `examples/` package and `salon-concierge-v2`, plus the golden files |
| the request block and the expected-value machinery are byte-identical on both targets, because both are rendered once above the drivers | `internal/generate/typed_inputs_test.go` (`TestRequestBlockIsByteIdenticalOnBothTargets`, `TestTypedInputMachineryIsByteIdenticalOnBothTargets`) |
| the values a step expects are validated before the step exists and before anything is spoken, refused naming the field with the previous contents standing, written to the shared state for the visit and **put back, never cleared**, after it, so an owner whose own brief shares a name keeps it; the restore does not run on the in-task handoff path, where the receiver's brief was just written | `internal/generate/typed_inputs_test.go` (`TestAnInputOutsideItsTypeIsRefusedBeforeTheStepIsConstructed`, `TestAStepsInputsAreSavedAndRestoredAroundTheVisit`) |
| a handoff validates its brief, resets every expected value the receiver's prompt reads, writes its own values and only then builds or activates the receiver, on the agent seam and the in-task seam alike; a receiving agent's brief survives its own steps; a Pipecat worker with a brief and no steps re-renders its prompt on activation | `internal/generate/typed_inputs_test.go` (`TestAHandoffWritesTheBriefBeforeTheReceiverExists`, `TestAReceivingAgentsBriefSurvivesItsOwnStep`, `TestPipecatWorkerWithABriefAndNoStepsRerendersOnActivation`) |
| a required expected value is required in both emitted schemas and an optional one is not, required parameters precede optional ones, and a Pipecat delegate or handoff with an `expect:` list advertises an explicit `FunctionSchema` spliced from the generated types while one without keeps the signature path, because that framework reads a schema off a Python signature and a signature cannot express a `Literal` or a declared class | `internal/generate/typed_inputs_test.go` (`TestInputsReachBothDelegateSchemas`) |
| every prompt naming an expected value reads whole with that value rendered as `not given.`; expected names join the router's template-variable names on both targets; the emitted runbook names the request block per receiver | `internal/generate/typed_inputs_test.go` (`TestInputPromptsReadWholeWithEveryValueEmpty`, `TestSlngRouterCarriesInputNamesOnBothTargets`, `TestRunbookNamesTheRequestBlockPerReceiver`) |
| `expect:` is refused on the slng target naming the target and the alternative, and the same package validates clean on both code targets | `internal/ir/input_test.go` (`TestSlngRefusesAnInput`) |
| `expect:` on a task and on a handoff round-trips through `unmute maintain`, both authored forms | `internal/tui/typed_inputs_roundtrip_test.go` (`TestMaintainKeepsATasksInputs`, `TestMaintainKeepsAHandoffsInputs`) |
| `internal/voice-agents-tests/salon-concierge-v3` declares `expect:` on its four seams, runs every step and handoff on `reset`, declares no `caller_reason` and no `reason:` result, and validates and generates on both targets, while `salon-concierge-v2` stays the frozen control | `internal/generate/examples_test.go` (`TestSalonConciergeV3HandsEverySeamItsRequest`, `TestSalonConciergeV2ScopesEveryStep`) |
| one scripted hand-in drives both emitted modules through a refused value, a required value left out, an optional value left out and a shaped value, and ends with identical state | `internal/generate/typed_inputs_smoke_test.go`, opt-in with `make smoke` |
...
| a prompt placeholder may name one part of a structured value through a dotted path, `{{customer.status}}`, at every site a placeholder is legal; the root follows every whole-value rule (undeclared, secret, expected-value scope, `confirm:`, greeting), an unknown field is refused listing the fields the shape declares, a path through a list and a path into a value with no fields are refused naming the root and what to write, and a token carrying anything but names is refused as an unknown field with the text as written | `internal/ir/path_test.go` (`TestBuildResolvesAPlaceholderPath`, `TestBuildResolvesAPathIntoAnExpectedValue`) |
...
session state and is the control, and `salon-concierge-v3` is the `expect:`
package, so a regression in one feature is measured against a package known to
work.
