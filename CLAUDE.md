...
| a handoff's `variables:` may be left out and then means `all`, an explicit empty list is refused saying what the two legal spellings mean, the same package compiles to the same bytes with and without `variables: all`, and the console writes the line back only when it was authored | `internal/ir/variables_optional_test.go`, `internal/generate/variables_optional_test.go`, `internal/tui/typed_inputs_roundtrip_test.go` (`TestMaintainKeepsAnOmittedVariablesOmitted`) |
