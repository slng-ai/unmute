...
targets verbatim, following `guard.go` and `prefetch.go`, so the two cannot
...
| an append on a value that is not a list is refused naming its declared type, and a `requires:` path that does not resolve in the declared shape is refused naming the fields that do | `internal/ir/shape_test.go` (`TestBuildRefusesAnAppendOnSomethingThatIsNotAList`, `TestBuildRefusesARequiresPathThatDoesNotResolve`) |
| an empty `list[...]` does **not** satisfy a `requires:` guard while `0` and `False` still do, and a path into an unconfirmed value satisfies nothing: the arm tests the declared type, because a blanket `if not value` regresses the case the predicate was written to protect | `internal/generate/guard_test.go` (`TestEmptyListDoesNotSatisfyAGuard`, `TestAPathIntoAnUnconfirmedValueSatisfiesNothing`) |
...
| any agent prompt and any task prompt may name any declared variable, written so the sentence reads whole when the value is empty, and `requires:` means one thing, wait for the value; only the greeting is held to a value settled before the first word | `internal/ir/variables_test.go` (`TestTaskPromptMayNameAVariableOnlyAnotherStepAssigns` and the agent-prompt case) |
